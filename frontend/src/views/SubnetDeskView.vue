...
import type { ControlOutcome, QuitOutcome } from '../../bindings/hanxi/internal/modules/subnetdesk/models'
...
const banner = computed(() => {
  ...
  if (state.value === 'failed') {
    return { cls: 'banner-error', text: snap.value?.error || 'SubnetDesk 异常退出' }
  }
...
async function openWindow() {
  ...
  busy.value = true
  try {
    const out: ControlOutcome = await SubnetDeskAPI.OpenWindow()
...
async function quitSubnetDesk() {
  ...
  busy.value = true
  try {
    const out: QuitOutcome = await SubnetDeskAPI.Quit()
    showToast(out.message)
    await refreshStatus()
  } catch (e) {
...
async function removeVersion(v: SDVersionInfo) {
  if (!window.confirm(`确定卸载 SubnetDesk ${v.version}？\n该版本隔离目录将被删除，不可恢复。\n（连接记录与 LAN 设置恒存于 %APPDATA%\\SubnetDesk，后续版本继续共用）`)) return
  try {
...
async function importLocal() {
  const path = window.prompt('请输入 SubnetDesk 便携 exe 的完整路径或其所在目录\n（官方形态文件名 subnetdesk-版本-x86_64.exe；安装版/MSI 不支持导入）\n提示：配置恒在 %APPDATA%\\SubnetDesk，与安装位置无关')
  if (!path) return
  try {
    busy.value = true
    const info = await SubnetDeskAPI.ImportLocal(path.trim())
    showToast(`已导入 SubnetDesk ${info.version}`)
    await loadVersions()
  } catch (e) {
...
async function onFollowToggle() {
  try {
    await SubnetDeskAPI.SetFollowOnExit(!followOnExit.value)
...
// ---------- 生命周期 ----------
onMounted(async () => {
  unlistenDownload = Events.On('subnetdesk:version-download', (event) => {
  ...
  unlistenState = Events.On('subnetdesk:instance-state', (event) => {
...
<template>
  <section class="page subnetdesk-view">
    <div class="header-row">
      <div>
        <h1>SubnetDesk 局域网远程桌面</h1>
        <p class="subtitle">托管 RustDesk 的 LAN 直连 fork：版本管理、JobObject 托管启停与窗口唤起；与「RustDesk 公网」互补共存。</p>
      </div>
      ...
      <table class="tbl">
        ...
        <tbody>
          <tr v-for="rel in releases" :key="rel.version">
            ...
            <td>
              <!-- 类名刻意用 sd-ver-status（含 sd- 前缀）——App.vue 全局样式有 .status-dot（7px），防碰撞压扁徽标 -->
...
.sd-status-light { width: 10px; height: 10px; border-radius: 50%; background: var(--text-subtle); flex-shrink: 0; }
.sd-status-light.running { background: var(--success); box-shadow: 0 0 0 3px rgba(26, 127, 55, 0.15); }
.sd-status-light.starting { background: var(--accent); animation: pulse 1s infinite; }
.sd-status-light.external { background: #9a6700; box-shadow: 0 0 0 3px rgba(154, 103, 0, 0.15); }
.sd-status-light.failed { background: var(--danger); box-shadow: 0 0 0 3px rgba(207, 34, 46, 0.15); }
...
.sd-ver-status.installed::before { background: #2da44e; }
.sd-ver-status.downloading::before { background: #0969da; animation: pulse 1s infinite; }
.sd-ver-status.error::before { background: #cf222e; }
.sd-ver-status.idle::before { background: #8c959f; }
