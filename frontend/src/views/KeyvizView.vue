...
const banner = computed(() => {
  ...
  if (state.value === 'failed') {
    return { cls: 'banner-error', text: snap.value?.error || 'Keyviz 异常退出' }
  }
...
async function removeVersion(v: KeyvizVersionInfo) {
  if (!window.confirm(`确定卸载 Keyviz ${v.version}？\n该版本托管目录将被删除，不可恢复。\n（你的 %APPDATA%\\org.keyviz 样式配置不受影响，后续版本继续共用）`)) return
  try {
...
async function importLocal() {
  const path = window.prompt('请输入本机 Keyviz 安装目录完整路径（含 keyviz.exe，如 C:\\Program Files\\keyviz）\n提示：配置恒在 %APPDATA%\\org.keyviz，与安装位置无关')
  if (!path) return
  try {
    busy.value = true
...
async function onFollowToggle() {
  try {
    await KeyvizAPI.SetFollowOnExit(!followOnExit.value)
    showToast(followOnExit.value ? '已开启：Hanxi 退出时一并关闭该工具' : '已关闭：Hanxi 退出不影响该工具，Keyviz 继续独立运行（下次启动生效）')
  } catch (e) {
    showToast('设置失败: ' + getErrorMessage(e))
...
<template>
  <section class="page keyviz-view">
    <div class="header-row">
      <div>
        <h1>Keyviz 键显</h1>
        <p class="subtitle">托管开源按键/鼠标可视化工具 Keyviz：官方 MSI 免安装提取、JobObject 启停与运行状态探测；样式设置在其托盘菜单 → Settings 完成。</p>
      </div>
        ...
        <table class="tbl">
          ...
          <tbody>
            <tr v-for="rel in releases" :key="rel.version">
              ...
              <td>
                <!-- 类名刻意用 kv- 前缀——App.vue 全局样式有 .status-dot（7px），防碰撞压扁徽标（markeron 垂直字体事故教训） -->
...
.kv-status-light { width: 10px; height: 10px; border-radius: 50%; background: var(--text-subtle); flex-shrink: 0; }
.kv-status-light.running { background: var(--success); box-shadow: 0 0 0 3px rgba(26, 127, 55, 0.15); }
.kv-status-light.starting { background: var(--accent); animation: pulse 1s infinite; }
.kv-status-light.external { background: #9a6700; box-shadow: 0 0 0 3px rgba(154, 103, 0, 0.15); }
.kv-status-light.failed { background: var(--danger); box-shadow: 0 0 0 3px rgba(207, 34, 46, 0.15); }
...
.kv-ver-status.installed::before { background: #2da44e; }
.kv-ver-status.downloading::before { background: #0969da; animation: pulse 1s infinite; }
.kv-ver-status.error::before { background: #cf222e; }
.kv-ver-status.idle::before { background: #8c959f; }
