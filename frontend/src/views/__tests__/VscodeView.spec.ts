...
// ---------- P0 批 3·4.4：请求代次（状态轮询与版本加载独立计数） ----------
describe('VSCodeView 请求代次', () => {
  it('状态轮询不废在途版本加载：该代仍是最新时慢回照写', async () => {
    vi.useFakeTimers()
    try {
      let releaseP!: (v: unknown[]) => void
      stubDefaults(statusOfForms(snap('stopped'), snap('stopped')))
      svc.ListRemoteVersions.mockImplementation((form: string) =>
        form === 'portable'
          ? (new Promise((r) => { releaseP = r }) as unknown as Promise<never>)
          : Promise.resolve([]))
      const { wrapper } = await mountView()
      await vi.advanceTimersByTimeAsync(2600) // 状态轮询过一轮，不得废掉在途 load
      releaseP([relP({ version: '1.139.9' })])
      for (let i = 0; i < 30; i++) await Promise.resolve()
      expect(wrapper.text()).toContain('1.139.9')
      wrapper.unmount()
    } finally {
      vi.useRealTimers()
    }
  ...
  it('旧 GetStatus 响应晚到不覆盖新状态', async () => {
    ...
    try {
      ...
      svc.GetStatus.mockImplementation(() => {
        call++
        // 吊慢第二发（首轮全部即时回 running 建立基线；5.2s 后新一轮轮询
        // 发起 call2 挂起，随后手动触发的控制动作刷新 call3 推前 statusSeq——
        // call2 慢回时旧代次必须被写权闸丢，状态不得倒回未运行）。
        if (call === 2) return new Promise((r) => { releaseSlow = r }) as unknown as Promise<never>
        return Promise.resolve(statusOfForms(
          snap('running', { version: '1.136.1', pid: 4242, startedAt: new Date().toISOString() }),
          snap('stopped'),
        ))
      })
      const { wrapper } = await mountView()
      // 轮询为尾调度：2.5s 首 tick 仍在吊慢的两发内，5s 后新一轮即时回 running
      await vi.advanceTimersByTimeAsync(5200)
      for (let i = 0; i < 30; i++) await Promise.resolve()
      expect(svc.GetStatus.mock.calls.length).toBeGreaterThanOrEqual(2)
      // 控制动作驱动的即时刷新推进代次（OpenWindow 成功 → store refresh）
      await svc.OpenWindow.mockResolvedValue({ action: 'opened', external: false, message: '' })
      const openBtn = bar(wrapper, 'portable').findAll('button').find(b => b.text().includes('打开'))
      if (openBtn) {
        await openBtn.trigger('click')
        for (let i = 0; i < 40; i++) await Promise.resolve()
      }
      releaseSlow(statusOfForms(snap('stopped'), snap('stopped'))) // 旧 call2 晚到
      await vi.advanceTimersByTimeAsync(50)
      for (let i = 0; i < 40; i++) await Promise.resolve()
      expect(bar(wrapper, 'portable').find('.status-word').text()).toBe('运行中')
      wrapper.unmount()
    } finally {
      vi.useRealTimers()
    }
