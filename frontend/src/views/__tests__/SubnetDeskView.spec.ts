// 特征测试（迁移基线）：SubnetDeskView 的托管家族契约锁定。
...
import SubnetDeskView from '../SubnetDeskView.vue'
...
vi.mock('../../../bindings/hanxi/internal/modules/subnetdesk/subnetdeskservice', () => svc)
...
async function mountView() {
  const Host = defineComponent({ render: () => h(KeepAlive, null, h(SubnetDeskView)) })
...
describe('SubnetDeskView 控制操作', () => {
  it('打开窗口：成功 toast 回执并立即刷新状态', async () => {
    stubDefaults({ state: 'stopped' })
    svc.OpenWindow.mockResolvedValue({ message: 'SubnetDesk 已启动并唤起窗口' })
    const w = await mountView()
    ...
    expect(useToast().toastMsg.value).toBe('SubnetDesk 已启动并唤起窗口')
...
describe('SubnetDeskView 版本管理', () => {
  ...
  it('导入本地：取消/空输入不动后端；有值去空格调 ImportLocal 并 toast', async () => {
    ...
    await w.find('.btn-group button').trigger('click')
    await flushMicrotasks()
    settlePrompt('  C:\\Downloads\\subnetdesk-1.2.3-x86_64.exe  ')
    await flushMicrotasks()
...
describe('SubnetDeskView 事件与轮询', () => {
  it('instance-state 事件即时改写界面', async () => {
    stubDefaults({ state: 'stopped' })
    const w = await mountView()
    runtime.handlers['subnetdesk:instance-state']({ data: { state: 'running', version: '1.9.9', pid: 9 } })
    await nextTick()
  ...
  it('download 事件驱动进度条百分比', async () => {
    ...
    const w = await mountView()
    runtime.handlers['subnetdesk:version-download']({ data: { version: '1.3.0', stage: 'downloading', done: 60, total: 100 } })
    await nextTick()
  ...
  it('激活后 2.5s 轮询刷新；停用后轮询静默且 uptime 归零', async () => {
    ...
    const show = ref(true)
    const Host = defineComponent({ render: () => (show.value ? h(KeepAlive, null, h(SubnetDeskView)) : h('div')) })
...
describe('SubnetDeskView 联动开关与仓库', () => {
