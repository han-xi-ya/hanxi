...
const installedSystem = {
  ...
  dir: 'C:\\Program Files\\RustDesk\\',
  size: 366960,
  installedAt: '2026-07-06 10:51:40',
}
...
describe('RustDeskView 双形态（安装版通道）', () => {
  it('安装版卡片：徽标/形态行/系统卸载引导（不直接调 RemoveVersion）', async () => {
    ...
    expect(sysCard.text()).toContain('安装版（Program Files + 系统服务）')
  ...
  it('设为使用：携带 form 参数（安装版/便携分流）', async () => {
    ...
    await flushMicrotasks()
    expect(svc.SetActiveVersion).toHaveBeenCalledWith('1.4.9', 'installed')
    w.unmount()
  })

  it('远程表双通道：installerName 存在才出安装版行；kind=installer 进度独立于便携 downloading', async () => {
    stubDefaults({ state: 'stopped' }, [], [
      { version: '1.4.9', size: 23300000, published: '2026-07-06T00:00:00Z', isPre: false, installerName: 'rustdesk-1.4.9-x86_64.msi', installerSize: 24911872 },
      { version: '1.4.6', size: 23100000, published: '2026-05-02T00:00:00Z', isPre: false, installerName: '' },
    ])
    const w = await mountView()
    // 1.4.9 行有安装通道按钮；1.4.6 行无
    const rows = w.findAll('tbody tr')
    expect(rows[0].text()).toContain('安装系统版')
    expect(rows[1].text()).not.toContain('安装系统版')
    expect(rows[0].find('.size-sub')?.text()).toContain('MSI')
    // installer 通道进度（stage=install 显示向导提示，而非进度条）
    svc.InstallVersion.mockResolvedValue('started')
    await rows[0].findAll('button').find(b => b.text() === '安装系统版')!.trigger('click')
    await flushMicrotasks()
    expect(svc.InstallVersion).toHaveBeenCalledWith('1.4.9')
    runtime.handlers['rustdesk:version-download']({ data: { version: '1.4.9', kind: 'installer', stage: 'install', done: 0, total: 0 } })
    await nextTick()
    expect(w.find('tbody tr').text()).toContain('安装向导已弹出')
    // 便携通道不被 installer 事件污染（无便携进度时仍显示下载按钮）
    expect(w.find('tbody tr').text()).toContain('下载便携版')
    w.unmount()
  })

  it('仅安装版已装时，便携通道不误显「已装」（形态精确匹配）', async () => {
    stubDefaults({ state: 'stopped' }, [installedSystem], [
      { version: '1.4.9', size: 23300000, published: '2026-07-06T00:00:00Z', isPre: false, installerName: 'rustdesk-1.4.9-x86_64.msi', installerSize: 24911872 },
    ])
    const w = await mountView()
    const row = w.find('tbody tr')
    expect(row.text()).toContain('下载便携版') // 便携通道仍可下载
    expect(row.text()).toContain('系统版已装') // 安装通道命中
    w.unmount()
  })

  it('运行态快照带 form=installed：形态徽标与安装版横幅文案', async () => {
    stubDefaults({ state: 'running', version: '1.4.9', form: 'installed', pid: 7 })
    const w = await mountView()
