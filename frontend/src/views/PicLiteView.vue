...
const releases = ref<PicRelease[]>([])
const installed = ref<PicVersionInfo[]>([])
...
async function loadVersions() {
  await loadManagedVersions({
    remote: PicLiteAPI.ListReleases,
    local: PicLiteAPI.ListInstalledVersions,
    active: PicLiteAPI.GetActiveVersion,
...
async function refreshStatus() {
  try {
    snap.value = await PicLiteAPI.GetStatus()
  } catch (e) {
    // 轮询静默失败：保留上次快照即可
...
function statusOf(rel: PicRelease): 'installed' | 'downloading' | 'error' | 'idle' {
...
async function openWindow() {
  ...
  busy.value = true
  try {
    const out: ControlOutcome = await PicLiteAPI.OpenWindow()
...
async function quitPicLite() {
  if (busy.value) return
  busy.value = true
  try {
    const out: QuitOutcome = await PicLiteAPI.Quit()
    showToast(out.message)
    await refreshStatus()
  } catch (e) {
...
// ---------- 版本管理操作 ----------
async function download(rel: PicRelease) {
  try {
...
async function setActive(v: PicVersionInfo) {
  try {
    const ver = await PicLiteAPI.SetActiveVersion(v.version)
...
async function importLocal() {
  ...
  try {
    ...
    showToast(`已导入 PicLite ${info.version}`)
    await loadVersions()
  } catch (e) {
...
async function loadExtras() {
  try {
    const [f, u] = await Promise.all([PicLiteAPI.GetFollowOnExit(), PicLiteAPI.RepositoryURL()])
...
async function onFollowToggle() {
  ...
  try {
    ...
    showToast(next ? '已开启：Hanxi 退出时一并关闭该工具' : '已关闭：Hanxi 退出不影响该工具，PicLite 继续独立运行（下次启动生效）')
  } catch (e) {
...
<template>
  <section class="page piclite-view">
    ...
    <div v-show="activeMainTab === 'console'" class="tab-body">
      ...
      <div class="control-bar">
        <div class="control-top">
          <div class="control-status">
            <span class="pl-status-light" :class="state"></span>
          ...
          <div class="control-btns">
            <button
              ...
              :title="state === 'running' ? '唤起主窗口（单实例协议）' : state === 'starting' ? '启动中…' : '启动 PicLite 并打开工作台窗口'"
              @click="openWindow"
            >🗔 打开窗口</button>
            <button
              class="btn btn-danger-outline btn-small"
              :disabled="busy || (state !== 'running' && state !== 'starting' && !isExternal)"
              :title="isExternal ? '外部实例请在 PicLite 托盘菜单退出' : '托管退出：直接终止实例（配置即时写盘不受影响；进行中的批量压缩会中断）'"
              @click="quitPicLite"
            >⏻ 退出</button>
          </div>
        </div>
      ...
      <div v-else-if="state === 'starting'" class="hint-line">正在拉起 PicLite（约 1~3 秒）…</div>

      <!-- 说明卡（可折叠） -->
      <details class="info-details">
        <summary class="info-summary">什么是 PicLite</summary>
        <div class="info-body">
          <p>开源跨平台本地图片/GIF 压缩工具（<a class="inline-link" href="https://github.com/amiaoapp/PicLite" target="_blank" rel="noopener">amiaoapp/PicLite</a>，GPL-3.0，Tauri 2）：智能择优压缩、悬浮结果窗、剪贴板/文件夹监测、图床上传，图片默认只在本机处理。</p>
          <p class="hint-dim">上游只发布安装器没有便携版：托管安装走 msiexec 管理提取（不写注册表、不提权），启停受 JobObject 管控；PicLite 关窗即驻托盘，退出无外部优雅通道，托管「退出」为直接终止。</p>
        </div>
      </details>
    </div>
    ...
    <div v-show="activeMainTab === 'versions'" class="tab-body">
      <div class="control-panel">
        <div class="meta-info">
          ...
          <span class="hint-dim">安装源为官方 GitHub Releases 的 Windows x64 MSI（官方 digest sha256 四层校验），经 msiexec 管理提取落进隔离目录，不触碰系统</span>
          <span class="hint-dim">「导入本地」可把你机器上已安装的 PicLite（Program Files\PicLite）收纳进托管；配置在 %APPDATA%，两者互不影响</span>
        </div>
        <div class="btn-group">
      ...
      <div v-if="installed.length === 0" class="empty-state first-use">
        <p>尚未安装 PicLite —— 下载官方 MSI 免安装提取，或「导入本地安装」把现有 PicLite 收纳进来</p>
        ...
        <div v-for="v in installed" :key="v.version" class="installed-card" :class="{ 'card-active': activeVersion === v.version }">
          ...
          <div class="inst-actions">
            ...
            <button
              ...
              :title="state === 'running' && runningVersion === v.version ? '请先退出 PicLite' : ''"
          ...
          <tbody>
            <tr v-for="rel in releases" :key="rel.version">
              ...
              <td>
                ...
                <span v-else class="pl-ver-status idle">可安装</span>
              </td>
...
.pl-ver-status { display: inline-flex; align-items: center; gap: 6px; font-size: var(--text-sm); white-space: nowrap; }
