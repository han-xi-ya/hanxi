...
async function loadVersions() {
  ...
  try {
    const [remote, local, active] = await Promise.all([
      PicLiteAPI.ListReleases(),
      PicLiteAPI.ListInstalledVersions(),
      PicLiteAPI.GetActiveVersion(),
    ])
...
async function onFollowToggle() {
  try {
    await PicLiteAPI.SetFollowOnExit(!followOnExit.value)
    showToast(followOnExit.value ? '已开启：Hanxi 退出时一并关闭该工具' : '已关闭：Hanxi 退出不影响该工具，PicLite 继续独立运行（下次启动生效）')
  } catch (e) {
    showToast('设置失败: ' + getErrorMessage(e))
