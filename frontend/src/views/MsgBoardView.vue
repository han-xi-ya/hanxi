...
// 桌面留言板模块页（重设计 v2，承接 N6 骨架重排）：以"挂牌动作"为绝对主角——
// 顶部主操作区聚合挂牌状态字、挂/撤大钮与"已存牌面"文案回显（挂牌真身走的
// 是服务端已存配置，回显绝不拿草稿冒充）；中部"挑一张牌面"类型卡保留既有契约
// （单击=填词进草稿、双击=保存并挂出）；草稿编辑与实时预览合并一卡，预览位即
// 编辑反馈位（所见即所得仍由 BoardCard 单组件同源保证）；字号/多屏/热键等低频
// 配置沉底折叠，异常时汇总警示（收编 ManagedConsoleShell 状态头语系原子与
// 设置页 setting-row 形制）。
// N30 补口原样保留：预览可放大为「全屏预览」纯前端浮层（同一 BoardCard 全尺寸
...
<template>
  <div class="page mb-page">
    ...
    <template v-else>
      <!-- 1. 主操作区（挂牌动作的绝对主角）：状态字 + 大挂撤钮 + 已存牌面回显。
           借用托管壳状态头语系原子（control-bar/control-top/status-word），异常
           chip 只在异常时出现——健康态这里只有"未挂牌/已挂牌"一件事。 -->
      <section class="control-bar mb-hero" aria-label="挂牌操作">
        ...
        <div class="hero-echo">
          ...
          <span v-else class="hero-echo-sub">（空牌——去下方"挑一张牌面"选一个，或在草稿里写字后保存）</span>
        </div>
        <p v-if="dirty" class="hero-dirty">{{ dirtyNote }}</p>
      </section>

      <!-- 保存失败整页可见（加载失败走上方的 state-error 闸口，这里只管动作失败） -->
      <section v-if="errorMsg && status" class="banner banner-error" role="alert">保存失败：{{ errorMsg }}</section>

      <!-- 2. 挑一张：单击=填词进草稿，双击=保存并挂出（既有契约不动） -->
      <section class="panel">
      ...
      <!-- 3. 草稿 + 预览合并一卡：预览位就是草稿所见的编辑反馈位 -->
      <section class="panel mb-work" aria-label="草稿与预览">
        <div class="panel-top">
          <h2 class="sec-title">草稿与预览</h2>
          <span v-if="dirty" class="chip chip-warning">未保存</span>
          <span v-else-if="savedTip" class="chip chip-positive">已保存</span>
        </div>
        <div class="mb-work-grid">
          <div class="mb-edit">
            <p class="sec-note">
          ...
          <div class="mb-previewzone" aria-label="牌面预览">
            <div class="mbp-box">
      ...
      <section class="panel usage">
        <details>
          ...
          <ul class="usage-list">
            ...
            <li>草稿卡的预览位可点「<b>全屏预览</b>」：在主窗内以浮层按真实大小渲染牌面观感（与真牌同一 BoardCard、同一压暗层），<kbd>Esc</kbd> 或点击即返回——纯预览动作，绝不写配置，与挂牌链路零交互。</li>
            <li>挂出期间系统不休眠、显示器不息屏（平台层引用计数聚合器，撤牌/停用/退出即释放）。</li>
            <li>撤牌即真销毁窗口、再唤即重建——不留隐藏窗占内存，也不得白边残影（踩坑 #50）。</li>
            <li>三条唤起通道：本页挂牌钮、全局热键（高级设置）、托盘右键/快捷轮盘命令。</li>
...
<style scoped>
/* 页体骨架：单一纵向流（主操作区→挑一张→草稿与预览→高级→说明），
   ≤760px 天然纵堆成同一顺序，无需结构重排 */
.mb-page { display: flex; flex-direction: column; gap: 12px; }
...
/* 主操作区：托管壳状态头语系的大号变体——状态字加圆点形状（不只靠颜色），
   挂撤钮抬高半档成页面唯一视觉主角；回显行只报已存牌面的真话 */
.mb-hero { padding: 14px 16px; gap: 10px; }
.hero-cta { min-height: 44px; padding: 8px 22px; font-size: var(--text-md); font-weight: 600; }
...
/* 挑一张：表情为主体、文字为副，双击手势在 title 与说明行双入口披露 */
...
/* 草稿与预览一卡：左编辑右预览，预览即编辑反馈位；窄于此宽度改上下堆叠 */
.mb-work-grid { display: grid; grid-template-columns: minmax(0, 1fr) 300px; gap: 16px; align-items: start; margin-top: 10px; }
.mb-edit { min-width: 0; display: flex; flex-direction: column; }
.mb-edit .sec-note { margin: 0 0 8px; }
.mb-previewzone { min-width: 0; }
.mb-text { resize: vertical; min-height: 96px; line-height: 1.6; font-family: inherit; }
...
/* 预览位：缩放盒按"牌的自然宽"排版不吃卡位挤压（防裁切换算见 previewScale）。
   缩放比只影响预览呈现——牌本体（popup）不吃这里的任何尺寸，
   同源的是 BoardCard 画法而非像素。 */
.mbp-box {
...
@media (max-width: 760px) {
  .mb-work-grid { grid-template-columns: minmax(0, 1fr); }
  .mbp-box { height: 240px; }
}
