...
/* ---------- 当前托管卡（installed-card/inst-card-top/inst-badges 基形由全局原子接管） ---------- */
/* 补差 against 全局原子 .installed-card：当前托管卡恒以主色描边（标准形为 --color-border） */
.installed-card { border-color: var(--color-primary); }
/* 补差 against 全局原子 .inst-card-top / .inst-badges：窄卡允许换行 */
.inst-card-top { flex-wrap: wrap; gap: 6px; }
...
.size-hint { font-size: var(--text-sm); min-width: 56px; }

.pt-ver-status { display: inline-flex; align-items: center; gap: 6px; font-size: var(--text-sm); white-space: nowrap; }
...
/* download-cell/dl-* 家族与 retry-link(:hover) 由全局原子接管（本视图原 dl-bar-inner
   "fast linear"、retry-link ml4 两处散差按标准形定档删除） */
/* 补差 against 全局原子 .dl-error：错误详情单行截断 */
.dl-error { max-width: 260px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
