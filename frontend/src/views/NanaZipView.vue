...
.nanazip-state-box{display:flex;align-items:center;gap:10px;padding:14px;border:1px dashed var(--color-border);border-radius:var(--radius-element);background:var(--surface-soft);font-size:var(--text-sm)}.nanazip-state-box span{color:var(--color-text-muted)}.nanazip-state-box button{margin-left:auto}.nanazip-state-box.error{border-style:solid;color:var(--state-danger)}
