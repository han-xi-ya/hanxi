...
import { createTermoraAdapter } from '../termora'
...
vi.mock('../../../bindings/hanxi/internal/modules/termora/termoraservice', () => svc)
...
describe('createTermoraAdapter', () => {
  beforeEach(() => vi.clearAllMocks())

  it('六态词表收口：quitting/external 一级公民，未知态兜底未托管', () => {
    const adapter = createTermoraAdapter()
  ...
  it('上游通道现实诚实披露：beta 即主干、双许可、自带 JRE 进 metaHints', () => {
    const adapter = createTermoraAdapter()
    const hints = adapter.copy?.metaHints ?? []
    const joined = hints.join(' ')
    expect(joined).toContain('预发布')
    expect(joined).toContain('AGPL')
    expect(joined).toContain('JRE')
  })
})
