...
func (s *AppService) SendTestNotification() {
	...
	notify.Success("system", product.Name+" 统一通知", "这是一条即时测试通知：窗口激活时为应用内卡片！", "/settings")
}
...
func (s *AppService) SendDelayedTestNotification(delaySeconds int) {
	...
	go func() {
		...
		notify.Success("system", product.Name+" 后台桌面通知", fmt.Sprintf("这是 %d 秒前触发的延迟通知：成功在后台/最小化时弹出 Windows 原生桌面气泡！", delaySeconds), "/settings")
	}()
}
