...
const (
	...
	dirPrefix        = "quicklook_"             // 版本隔离目录前缀（与 ccswitch_ / keyviz_ 同构）
...
// dirNameRe 版本目录名（quicklook_4.5.0）；imported- 分支收纳版本探测失败的导入
var dirNameRe = regexp.MustCompile(`^` + dirPrefix + `(?:[0-9][0-9a-zA-Z.]+|imported-\d{8}-\d{6})$`)

// Manager QuickLook 版本管理引擎：远程列表、下载完整性校验、保布局解压隔离、本地整套导入。
type Manager struct {
	versionsDir string
	client      *http.Client // 下载客户端（长超时，便携 zip 约 117MB）
}
...
// exe 缺失/为空，或便携标记缺失，均视为损坏安装跳过（官方便携 zip 根恒含二者）。
...
// Download 下载便携 zip 并解压安装到 versions/quicklook_X.Y.Z/。
// 上游提供官方 sha256（GitHub API digest），完整性四层兜底（与 ccswitch 同构）：
//  1. 官方 sha256 校验（第一主依据）；
//  2. 下载落盘字节数 == release API 声明的 size（防截断/代理篡改）；
//  3. archive/zip 读取每个 entry 时强制 CRC32 校验（extractAll 读满不提前返回）；
//  4. 解压后布局自检（QuickLook.exe 非空 + portable.lock + 原生 DLL 存在），失败清理目录。
//
