...
// 与启动恢复"背书"清理。解包段 bespoke（QuickLook longpath/反斜杠特例）
// 同样记账——事务与解包器无关（ADR-0002 §5 / ADR-0003）。
...
// quicklookTestSteps 与 QuickLookService.quicklookInstallSteps 同一词表（服务层
// 编排在本包不可达，此处复刻同款阶段映射以验证交接面；官方摘要双核由内核
// Fetch 原子折进 download 步，journal 不造幻影 verify 步）。
var quicklookTestSteps = []string{"download", "unpack", "place"}
...
func driveInstall(t *testing.T, m *Manager, txnID, version string) error {
	t.Helper()
	txn, err := ops.BeginTxn("quicklook", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		version, txnID, quicklookTestSteps)
	if err != nil {
		return err
	}
	stepIdx := -1
...
func TestJournalChainSuccess(t *testing.T) {
	...
	if err := driveInstall(t, m, txnID, "4.5.0"); err != nil {
	...
	if j.State != operation.TxnSucceeded || j.Operation != operation.TxnOpInstall ||
		j.DeliveryKind != extapi.DeliveryManagedDeclarative || j.ModuleID != "quicklook" {
		t.Errorf("账本终态异常: %+v", j)
	}
...
func TestJournalChainFailurePlaceAttribution(t *testing.T) {
	...
	src := newZipSource(t)
	m := newChainManager(t, src)
	installViaChain(t, m, src, "4.5.0", "same-exe")
...
func TestJournalSingleWriteRejectsConcurrent(t *testing.T) {
	withTestKernel(t)
	if _, err := ops.BeginTxn("quicklook", extapi.OpInstall, extapi.DeliveryManagedDeclarative, "4.4.0", newTxnID(), quicklookTestSteps); err != nil {
		t.Fatal(err)
	}
	_, err := ops.BeginTxn("quicklook", extapi.OpUpdate, extapi.DeliveryManagedDeclarative, "4.5.0", newTxnID(), quicklookTestSteps)
...
func TestJournalCrashRecoveryEndorsement(t *testing.T) {
	...
	txnID := newTxnID()
	txn, err := ops.BeginTxn("quicklook", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		"4.5.0", txnID, quicklookTestSteps)
	if err != nil {
		t.Fatal(err)
	}
	txn.Step(0)
	...
	trees := map[string]*artifact.Tree{"quicklook": OpenTree(m.versionsDir)}
