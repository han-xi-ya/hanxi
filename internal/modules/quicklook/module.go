...
func New(plat platform.Platform) extapi.Module {
	return &Module{svc: NewQuickLookService(plat, extapi.NewLeaseHolder(ID))}
}
...
func (m *Module) TrayCommands() []extapi.TrayCommand {
	return []extapi.TrayCommand{
		{ID: "launch", Label: "启动 QuickLook", Run: func(context.Context) error {
			_, err := m.svc.StartQuickLook()
			return err
		}},
	}
}
