...
// litemonitorStore 持久化 LiteMonitor 少量偏好：
// 位置 <stateDir>/litemonitor.json，仅存 activeVersion（空字符串 = 未指定，冷启动自动回退最新已装）。
// 原子写（tmp+rename）与损坏容忍（解析失败按空配置继续，不阻断模块）收口至 internal/jsonstore。
type litemonitorStore struct {
	filePath      string
	mu            sync.RWMutex
...
func (s *litemonitorStore) load() error {
	s.mu.Lock()
	defer s.mu.Unlock()

	var cfg litemonitorConfig
...
func (s *litemonitorStore) saveLocked() error {
	return jsonstore.Save(s.filePath, litemonitorConfig{ActiveVersion: s.activeVersion, FollowOnExit: &s.followOnExit})
}

// GetActive 返回当前设定版本（空字符串 = 未指定）。
func (s *litemonitorStore) GetActive() string {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *litemonitorStore) SetActive(version string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *litemonitorStore) SetFollowOnExit(b bool) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.followOnExit = b
	return s.saveLocked()
