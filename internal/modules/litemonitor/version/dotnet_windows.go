...
// 不依赖 dotnet CLI（PATH 里未必有），直接读磁盘目录得出权威结论（bcu/papertodo 同款策略）。
const dotnetInstallRoot = `C:\Program Files\dotnet`

// RequiresDesktopMajor LiteMonitor 主工程 TargetFramework=net8.0-windows 且
// 发布 zip 不含 runtime DLL（框架依赖版）——需要 .NET 8 桌面运行时。
...
// HasDesktopRuntimeMajor 已装桌面运行时中是否存在主版本为 major 的（"8" → 8.0.x）。
// .NET 运行时不跨大版本回退：net8.0-windows 应用需要 8.x 桌面运行时，
// 只装了 10.x 也跑不起来。
