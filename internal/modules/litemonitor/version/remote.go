...
// 资产名形如 LiteMonitor_v1.3.6-win-x64.zip；后缀兜底 "-win-x64.zip"
// 天然排除未来可能出现的 arm64/msi/安装包变体。
func findPortableAsset(assets []asset, version string) (asset, bool) {
	ver := strings.TrimPrefix(version, "v")
	needName := strings.ToLower("litemonitor_v" + ver + "-win-x64.zip")
	...
	for _, a := range assets {
		...
		if strings.HasSuffix(lower, "-win-x64.zip") && strings.Contains(lower, strings.ToLower(ver)) {
			return a, true
		}
	}
	return asset{}, false
...
func (c *releaseCache) get() ([]LMRelease, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func parseReleasesBody(body []byte) ([]LMRelease, error) {
