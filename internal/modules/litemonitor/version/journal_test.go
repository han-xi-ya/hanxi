...
// litemonitorTestSteps 与 LiteMonitorService.litemonitorInstallSteps 同一词表（服务层
// 编排在本包不可达，此处复刻同款阶段映射以验证交接面；verify 由内核 Fetch
// 折进 download 步，不造幻影步骤）。
var litemonitorTestSteps = []string{"download", "unpack", "place"}
...
func driveInstall(t *testing.T, m *Manager, txnID, version string) error {
	t.Helper()
	txn, err := ops.BeginTxn("litemonitor", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		version, txnID, litemonitorTestSteps)
	if err != nil {
		return err
	}
	stepIdx := -1
...
func TestJournalChainSuccess(t *testing.T) {
	...
	if err := driveInstall(t, m, txnID, "v1.3.6"); err != nil {
	...
	if j.State != operation.TxnSucceeded || j.Operation != operation.TxnOpInstall ||
		j.DeliveryKind != extapi.DeliveryManagedDeclarative || j.ModuleID != "litemonitor" {
		t.Errorf("账本终态异常: %+v", j)
	}
...
func TestJournalSingleWriteRejectsConcurrent(t *testing.T) {
	withTestKernel(t)
	if _, err := ops.BeginTxn("litemonitor", extapi.OpInstall, extapi.DeliveryManagedDeclarative, "v1.3.5", newTxnID(), litemonitorTestSteps); err != nil {
		t.Fatal(err)
	}
	_, err := ops.BeginTxn("litemonitor", extapi.OpUpdate, extapi.DeliveryManagedDeclarative, "v1.3.6", newTxnID(), litemonitorTestSteps)
...
// （背书清理生效的前提是把本模块树登记进装配根 versionTrees——litemonitor
// 尚未登记，本用例以本地 trees 映射验证交接面契约，登记动作待装配根接线。）
func TestJournalCrashRecoveryEndorsement(t *testing.T) {
	...
	txnID := newTxnID()
	txn, err := ops.BeginTxn("litemonitor", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		"v1.3.6", txnID, litemonitorTestSteps)
	if err != nil {
		t.Fatal(err)
	}
	txn.Step(0)
	...
	trees := map[string]*artifact.Tree{"litemonitor": OpenTree(m.versionsDir)}
