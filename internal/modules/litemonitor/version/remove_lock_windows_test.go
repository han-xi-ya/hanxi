...
// TestRemoveRefusedWhileExeLocked 文件锁拒卸：版本目录内 LiteMonitor.exe 被非共享
...
func TestRemoveRefusedWhileExeLocked(t *testing.T) {
	...
	exePath := filepath.Join(m.versionsDir, "litemonitor_1.3.5", exeName)
	...
	if rmErr := m.Remove("v1.3.5"); rmErr == nil {
