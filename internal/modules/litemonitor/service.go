...
func (s *LiteMonitorService) DownloadVersion(targetVersion string) (string, error) {
	...
	go func() {
		...
		if lerr != nil {
			if app := application.Get(); app != nil && app.Event != nil {
				app.Event.Emit("litemonitor:version-download", version.DownloadProgress{Version: targetVersion, Stage: "error", Message: lerr.Error()})
			}
			notify.Error("litemonitor", "版本下载失败", fmt.Sprintf("LiteMonitor %s 后台租约开启失败: %v", targetVersion, lerr), "/ext/litemonitor")
			return
		}
		...
		if err := txn.Done(); err != nil {
			emit(version.DownloadProgress{Version: targetVersion, Stage: "error", Message: err.Error()})
			notify.Error("litemonitor", "版本下载失败", fmt.Sprintf("LiteMonitor %s 事务收口失败: %v", targetVersion, err), "/ext/litemonitor")
			return
		}
...
func (s *LiteMonitorService) ImportLocal(srcDir string) (version.LMVersionInfo, error) {
	...
	if err != nil {
		return version.LMVersionInfo{}, err
	}
