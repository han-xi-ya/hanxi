package litemonitor

import (
	"os"
	"path/filepath"
	"testing"
)

// TestFollowOnExitDefaultAndPersist 联动开关：缺省 false（默认不随 Hanxi 关闭），
// 设 true 后落盘重载仍为 true；存取与 ActiveVersion 互不干扰。
func TestFollowOnExitDefaultAndPersist(t *testing.T) {
	dir := t.TempDir()

	s := newLiteMonitorStore(dir)
	if s.GetFollowOnExit() {
		t.Fatal("新 store 的 followOnExit 默认应为 false（不随 Hanxi 关闭）")
	}

	if err := s.SetFollowOnExit(true); err != nil {
		t.Fatalf("SetFollowOnExit: %v", err)
	}
	s2 := newLiteMonitorStore(dir)
	if !s2.GetFollowOnExit() {
		t.Fatal("重载后 followOnExit 应保持 true")
	}

	// SetActive 不影响联动开关
	if err := s.SetActive("v1.3.6"); err != nil {
		t.Fatalf("SetActive: %v", err)
	}
	if !s.GetFollowOnExit() {
		t.Fatal("SetActive 不应影响 followOnExit")
	}
}

// TestStoreActiveVersionCorruptTolerant 损坏容忍：非法 JSON 按空配置继续，不阻断。
func TestStoreActiveVersionCorruptTolerant(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "litemonitor.json"), []byte("{-not-json"), 0644); err != nil {
		t.Fatal(err)
	}
	s := newLiteMonitorStore(dir)
	if s.GetActive() != "" {
		t.Errorf("损坏配置应回落空 activeVersion，实际 %q", s.GetActive())
	}
	if s.GetFollowOnExit() {
		t.Error("损坏配置下联动开关应回落默认 false")
	}
	// 损坏后仍可正常覆写
	if err := s.SetActive("v1.3.5"); err != nil {
		t.Fatalf("SetActive: %v", err)
	}
	if newLiteMonitorStore(dir).GetActive() != "v1.3.5" {
		t.Error("覆写后重载应生效")
	}
}
