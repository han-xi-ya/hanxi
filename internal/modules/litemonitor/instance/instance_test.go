...
func TestStartCreateJobFail(t *testing.T) {
	...
	err := e.Start(StartOptions{Version: "v1.3.6", Exe: mustCmdExe(t)})
	if err == nil {
...
// TestWaitExternalTaken 冷启动竞速：我们的进程抢锁失败自退（exit 0）后
// 进程枚举仍命中外部实例 → external。
...
// TestWaitAbnormalExit 异常退出码（1）→ failed 且错误提示指向 .NET 运行时。
func TestWaitAbnormalExit(t *testing.T) {
	...
	if !strings.Contains(snap.Error, ".NET 8") {
		t.Errorf("失败文案应指向 .NET 8 桌面运行时: %q", snap.Error)
	}
}
...
// TestQuitGraceFallback 冒烟：Quit 宽限路径——ping 长命进程走强杀兜底，
// 短命进程走宽限内自然退出的优雅路径。
