...
// 覆盖"窗口被 HideMainForm 隐藏后信使路径不可达"等罕见场。
...
// Start 启动自有实例：播种托管配置 → 创建进程 → 绑定 JobObject → 状态 running。
// LiteMonitor 无后台启动 CLI，唯一启动语义即"无参拉起 → 主监控条显示"。
// 本方法不做单实例探测：冷启动与外部实例竞速的 TOCTOU 交给 wait() 退出分类兜底
// （第二实例抢锁失败静默 exit 0 → 进程枚举命中外部实例 → external）。
func (e *Engine) Start(opts StartOptions) error {
	...
	// LiteMonitor 是 GUI 子系统程序，无需隐藏控制台窗口。
	cmd := exec.Command(opts.Exe)
	cmd.Dir = filepath.Dir(opts.Exe) // 工作目录锁定：settings.json/themes 按 BaseDirectory 解析，cwd 同步锚定
...
//  1. EnumWindows 按 PID 向全部顶层窗口投递 WM_CLOSE——LiteMonitor 退出菜单
//     即 form.Close() 且无 FormClosing 拦截，关窗即退进程；
//  2. 宽限 closeGracePeriod 轮询进程自然退出；
//  3. 超时（窗口未创建/模态对话框阻塞消息循环等）JobObject 强杀兜底——
//     settings.json 上游为 tmp+rename 原子写 + .bak 备份，进程级终止安全。
...
func (e *Engine) wait() {
	...
	// 分支顺序不可换：冷启动竞速场我们的进程抢锁失败静默自退（exit 0），
	// 必须先判外部接管——进程枚举仍命中说明真正存活的是外部实例
	...
	case code == 0 && prev == StateRunning:
		e.transition(StateStopped, "") // 用户在 LiteMonitor 托盘/菜单自己退出
	default:
		e.transition(StateFailed, fmt.Sprintf("LiteMonitor 异常退出（退出码 %d）。请确认已安装 .NET 8 桌面运行时，或尝试重新安装该版本", code))
	}
}
