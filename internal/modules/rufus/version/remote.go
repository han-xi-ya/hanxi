...
// 拉取 GitHub API 的候选前缀：直连优先，镜像逐个回退（与 ccswitch/litemonitor 同组）
...
// plainSemverTag 严格纯语义版本 tag（格式 vX.Y）。
// Rufus 上游自 1.x 起恒为两段式版本（v4.15 等），beta 预发布 tag（v4.5b1 形态）
// 与非规范 tag 一律丢弃，防污染版本列表——托管只收稳定版。
var plainSemverTag = regexp.MustCompile(`^v\d+\.\d+$`)
...
// findPortableAsset 从 release 资产中筛选 Windows x64 便携单文件 exe。
// 资产名形如 rufus-4.15p.exe；同 release 还含 rufus-4.15.exe（安装版，与便携版
// 字节级同哈希）、rufus-4.15_x86.exe / _arm64.exe 架构变体与 .exe.sig（GPG 签名）。
// "p.exe" 结尾精确命中便携版，天然排除 sig（.sig 结尾）与架构变体（_x86/_arm64 结尾）；
// 若上游某版只发安装版，退一步收 rufus-X.Y.exe 精确名（实证两形态同二进制）。
func findPortableAsset(assets []asset, version string) (asset, bool) {
	ver := strings.TrimPrefix(version, "v")
	needName := strings.ToLower("rufus-" + ver + "p.exe")
	installerName := strings.ToLower("rufus-" + ver + ".exe")
...
func (c *releaseCache) get() ([]RufusRelease, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// 过滤规则：draft / 非纯语义两段式 tag（beta 等）/ 无 x64 便携资产 /
// 官方 sha256 缺失的 release 一律不入列表。
func parseReleasesBody(body []byte) ([]RufusRelease, error) {
	var releases []release
	if err := json.Unmarshal(body, &releases); err != nil {
		return nil, fmt.Errorf("parse releases: %w", err)
	}

	var list []RufusRelease
	for _, r := range releases {
		// 未认证 API 本就不返回 draft，防御性再跳一次；tag 非纯 vX.Y 直接丢弃
		...
		list = append(list, RufusRelease{
			Version:   r.TagName, // tag 已带 v 前缀，显示/目录直接规范成 vX.Y
