...
func (s *RufusService) DownloadVersion(targetVersion string) (string, error) {
	...
	go func() {
		...
		if lerr != nil {
			if app := application.Get(); app != nil && app.Event != nil {
				app.Event.Emit("rufus:version-download", version.DownloadProgress{Version: targetVersion, Stage: "error", Message: lerr.Error()})
			}
			notify.Error("rufus", "版本下载失败", fmt.Sprintf("Rufus %s 后台租约开启失败: %v", targetVersion, lerr), "/ext/rufus")
			return
		}
		...
		if err := txn.Done(); err != nil {
			emit(version.DownloadProgress{Version: targetVersion, Stage: "error", Message: err.Error()})
			notify.Error("rufus", "版本下载失败", fmt.Sprintf("Rufus %s 事务收口失败: %v", targetVersion, err), "/ext/rufus")
			return
		}
...
func (s *RufusService) ImportLocal(srcPath string) (version.RufusVersionInfo, error) {
	...
	if err != nil {
		return version.RufusVersionInfo{}, err
	}
