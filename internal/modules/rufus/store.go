...
// 与 followOnExit 开关。与 litemonitorStore/ccswitchStore 同款原子写（tmp+rename），
// 损坏容忍（解析失败按空配置继续，不阻断模块）。
...
func newRufusStore(dir string) *rufusStore {
	s := &rufusStore{filePath: filepath.Join(dir, "rufus.json"), followOnExit: true}
	_ = s.load()
	return s
}
...
func (s *rufusStore) saveLocked() error {
	...
	bytes, err := json.MarshalIndent(rufusConfig{ActiveVersion: s.activeVersion, FollowOnExit: &s.followOnExit}, "", "  ")
	if err != nil {
		return err
	}
