...
// Start 启动自有实例：播种便携配置 → 创建进程 → 绑定 JobObject → 状态 running。
// Rufus 无后台启动 CLI，唯一启动语义即"无参拉起 → 主对话框显示"。
// 本方法不做单实例探测：冷启动与外部实例竞速的 TOCTOU 交给 wait() 退出分类兜底
// （第二实例抢锁失败弹模态错误框退出 → 进程枚举/互斥体仍命中外部实例 → external）。
func (e *Engine) Start(opts StartOptions) error {
	...
	// 首启种子配置：激活便携模式并关闭上游内置更新检查（仅 rufus.ini 不存在时写入）。
	// 失败不阻断启动，最坏回到上游默认行为（设置落注册表 + 弹更新检查）。
	_ = seedPortableSettings(filepath.Dir(opts.Exe))

	// Rufus 是 GUI 子系统程序，无需隐藏控制台窗口。
	cmd := exec.Command(opts.Exe)
	cmd.Dir = filepath.Dir(opts.Exe) // 工作目录锁定：便携 ini 按 exe 目录解析，cwd 同步锚定
...
//  1. EnumWindows 按 PID 向全部顶层窗口投递 WM_CLOSE——Rufus 关主对话框即退进程；
//  2. 宽限 closeGracePeriod 轮询进程自然退出；
//  3. 超时（写盘确认对话框挂住消息循环等）JobObject 强杀兜底——
//     ⚠ 若此刻正在写盘，强杀会产出半成品盘；正常路径用户在窗口内自己确认，
//     前端常态展示"写入中勿退出"警示条（见 RufusView）。
...
func (e *Engine) wait() {
	...
	// 分支顺序不可换：冷启动竞速场我们的进程抢锁失败弹框退出后，
	// 必须先判外部接管——互斥体/进程枚举仍命中说明真正存活的是外部实例
	...
	case code == 0 && prev == StateRunning:
		e.transition(StateStopped, "") // 用户在 Rufus 窗口自己关闭了程序
	default:
		e.transition(StateFailed, fmt.Sprintf("Rufus 异常退出（退出码 %d）。若窗口从未出现，请确认 Hanxi 正以管理员身份运行（Rufus 清单强制提权），或尝试重新安装该版本", code))
	}
}
