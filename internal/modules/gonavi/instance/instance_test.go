...
// 本文件聚焦 gonavi 适配层：内核快照 → 本包 Snapshot 的形状/词表映射、
// ErrExternal 契约映射、"已手动停止"文案折回、Inspect 适配器的 PID 充实、
// 唤窗直操作与 WM_CLOSE 宽限收口，以及维持既有环境口径的真进程冒烟。
...
func TestExitCodeAndMessageMapping(t *testing.T) {
	...
	e.onSupState(sup.Snapshot{State: sup.StateFailed, PID: 0, Version: "v0.8.9",
		Error: "托管进程异常退出（退出码 1）"})
	snap := rec.last()
	if snap.State != StateFailed || snap.ExitCode != 1 {
		t.Fatalf("failed 映射异常: %+v", snap)
	}
	if !strings.Contains(snap.Error, "GoNavi 异常退出（退出码 1）") || !strings.Contains(snap.Error, "WebView2") {
		t.Errorf("异常退出文案应保持 gonavi 既有口径: %s", snap.Error)
	}
...
func TestSupProbeInspectOwnPidInfo(t *testing.T) {
	...
	if err != nil || !running || info == nil || info.PID != 4242 || info.Name != exeImageName {
...
func TestStartEmptyExeRejected(t *testing.T) {
	...
	if err := e.Start(StartOptions{Version: "v0.8.9"}); err == nil ||
		err.Error() != "GoNavi.exe 路径不能为空" {
...
func TestStartCreateJobFail(t *testing.T) {
	...
	err := e.Start(StartOptions{Version: "v0.8.9", Exe: mustCmdExe(t)})
	if err == nil {
...
// （无窗口进程 no-op 但应重播快照）→ Stop 强杀 → stopped。
// gonavi 刻意无信使路径（二次拉起即多开），唤窗只有直操作一条腿。
func TestManagedLifecycleSmoke(t *testing.T) {
	...
	exe := mustCmdExe(t)
	if err := e.Start(StartOptions{Version: "v0.8.9", Exe: exe}); err != nil {
	...
	// 唤窗直操作：无窗口进程上 no-op 但必须重播当前快照（RestoreWindow 语义）
	before := rec.count()
	e.RestoreWindow()
	if rec.count() <= before {
...
// TestRestoreWindowStoppedNoop 静止态唤窗：不广播（与家族实现一致）。
...
// （真 GoNavi"未保存 SQL 模态框挂住消息循环"的场即走此兜底路径。）
func TestQuitGraceFallback(t *testing.T) {
	...
	e.specArgs = smokeCmdArgs
	if err := e.Start(StartOptions{Version: "v0.8.9", Exe: mustCmdExe(t)}); err != nil {
		t.Fatalf("Start: %v", err)
	}
	defer func() { _ = e.Stop() }()
	waitState(t, e, StateRunning)
...
// 按 gonavi 既有契约映射为成功无操作，状态校正为 external 且 PID 由探针充实。
...
func TestProbePassthrough(t *testing.T) {
	...
	if err := e2.Start(StartOptions{Version: "v0.8.9", Exe: mustCmdExe(t)}); err != nil {
		t.Fatalf("Start: %v", err)
	}
