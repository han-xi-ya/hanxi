...
// 位置 <stateDir>/markeron.json，仅存 activeVersion（空字符串 = 未指定，冷启动自动回退最新已装）。
...
func newMarkeronStore(dir string) *markeronStore {
	s := &markeronStore{filePath: filepath.Join(dir, "markeron.json"), followOnExit: false}
	_ = s.load()
	return s
}
...
func (s *markeronStore) saveLocked() error {
	return jsonstore.Save(s.filePath, markeronConfig{ActiveVersion: s.activeVersion, FollowOnExit: &s.followOnExit})
}
