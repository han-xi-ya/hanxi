// Wave 4-B 安装事务 journal 链路测试（双样本之二，与 rufus 同构、资产形态为
// 便携 zip）：假下载源驱动 Begin→Advance→Complete 全步、中断失败收口、强杀模拟
// （重启 Store 后 Pending 可见）与启动恢复"背书"清理。全部真 tmp 目录 + 回环
// 假源，无网络依赖。
package version
...
// markeronTestSteps 与 MarkerOnService.markeronInstallSteps 同一词表（服务层
// 编排在本包不可达，此处复刻同款阶段映射以验证交接面；verify 由内核 Fetch
// 折进 download 步，不造幻影步骤）。
var markeronTestSteps = []string{"download", "unpack", "place"}
...
func driveInstall(t *testing.T, m *Manager, txnID, version string) error {
	t.Helper()
	txn, err := ops.BeginTxn("markeron", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		version, txnID, markeronTestSteps)
	if err != nil {
		return err
	}
	stepIdx := -1
...
func TestJournalChainSuccess(t *testing.T) {
	...
	if j.State != operation.TxnSucceeded || j.Operation != operation.TxnOpInstall ||
		j.DeliveryKind != extapi.DeliveryManagedDeclarative || j.ModuleID != "markeron" {
		t.Errorf("账本终态异常: %+v", j)
	}
...
func TestJournalChainFailureInterrupt(t *testing.T) {
	...
	src := newZipSource(t)
	m := newChainManager(t, src)
	installViaChain(t, m, src, "v0.9.2", "old-exe")
...
func TestJournalSingleWriteRejectsConcurrent(t *testing.T) {
	withTestKernel(t)
	if _, err := ops.BeginTxn("markeron", extapi.OpInstall, extapi.DeliveryManagedDeclarative, "v0.9.2", newTxnID(), markeronTestSteps); err != nil {
		t.Fatal(err)
	}
	_, err := ops.BeginTxn("markeron", extapi.OpUpdate, extapi.DeliveryManagedDeclarative, "v0.10.0", newTxnID(), markeronTestSteps)
...
func TestJournalCrashRecoveryEndorsement(t *testing.T) {
	...
	txnID := newTxnID()
	txn, err := ops.BeginTxn("markeron", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		"v0.10.0", txnID, markeronTestSteps)
	if err != nil {
		t.Fatal(err)
	}
	txn.Step(0)
	...
	trees := map[string]*artifact.Tree{"markeron": OpenTree(m.versionsDir)}
