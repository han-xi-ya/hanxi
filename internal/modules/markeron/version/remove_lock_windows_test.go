...
// TestRemoveRefusedWhileExeLocked 文件锁拒卸：版本目录内 MarkerOn.exe 被非共享
...
func TestRemoveRefusedWhileExeLocked(t *testing.T) {
	...
	installViaChain(t, m, src, "v0.9.2", "locked-exe")

	exePath := filepath.Join(m.versionsDir, "markeron_0.9.2", exeName)
	...
	if rmErr := m.Remove("v0.9.2"); rmErr == nil {
	...
	if err := m.Remove("v0.9.2"); err != nil {
