...
// seedRemote 注入伪远程列表与官方摘要。remoteCache 是包内全局缓存（生产语义即
// 进程级单例），本包测试串行执行（不使用 t.Parallel），互相隔离靠 setup 重灌。
func seedRemote(t *testing.T, version, assetName string, size int64, digest string) {
	t.Helper()
	remoteCache.mu.Lock()
	defer remoteCache.mu.Unlock()
	remoteCache.data = []MarkerRelease{{
		Version:   version,
		AssetName: assetName,
...
func newChainManager(t *testing.T, src *zipSource) *Manager {
	...
	m.mirrors = func(_, _ string) []string {
		return []string{src.srv.URL + "/MarkerOn_x64_portable.zip"}
	}
	return m
}
...
func installViaChain(t *testing.T, m *Manager, src *zipSource, version, exeContent string) {
	...
	seedRemote(t, version, "MarkerOn_"+strings.TrimPrefix(version, "v")+"_x64_portable.zip",
...
func TestDownloadInterruptKeepsOldVersion(t *testing.T) {
	...
	installViaChain(t, m, src, "v0.9.2", "old-exe-bytes")

	newZip := buildPortableZip(t, "brand-new-exe")
	seedRemote(t, "v0.10.0", "MarkerOn_0.10.0_x64_portable.zip", int64(len(newZip)), shaHex(newZip))
	src.set(newZip, true) // 半路掐线

	rec := &progressRecorder{}
	err := m.Download(newTxnID(), "v0.10.0", rec.cb())
	if err == nil {
		t.Fatal("下载中断应报错")
	}
	stages := rec.stages()
	...
	if _, statErr := os.Stat(filepath.Join(m.versionsDir, "markeron_0.10.0")); !os.IsNotExist(statErr) {
		t.Error("新半件目录不得出现")
	}
	assertNoTransactionLeftovers(t, m.versionsDir)
	installed, lerr := m.ListInstalled()
	if lerr != nil || len(installed) != 1 || installed[0].Version != "v0.9.2" {
...
func TestDownloadBadDigestRejects(t *testing.T) {
	...
	m := newChainManager(t, src)
	installViaChain(t, m, src, "v0.9.2", "old-exe-bytes")
	oldExeHashBefore, _ := fileSHA256(filepath.Join(m.versionsDir, "markeron_0.9.2", exeName))

	newZip := buildPortableZip(t, "tampered-payload")
	seedRemote(t, "v0.10.0", "MarkerOn_0.10.0_x64_portable.zip", int64(len(newZip)),
		shaHex([]byte("NOT-THE-REAL-DIGEST"))) // 官方摘要对不上任何传输字节
	src.set(newZip, false)

	err := m.Download(newTxnID(), "v0.10.0", nil)
	if err == nil {
		t.Fatal("摘要不符应拒收")
	}
	...
	assertNoTransactionLeftovers(t, m.versionsDir)
	oldExeHashAfter, _ := fileSHA256(filepath.Join(m.versionsDir, "markeron_0.9.2", exeName))
...
func TestDownloadMissingDigestRefusesUnverified(t *testing.T) {
	...
	seedRemote(t, "v0.10.0", "MarkerOn_0.10.0_x64_portable.zip", int64(len(zipBytes)), "")
	src.set(zipBytes, false)
...
// TestUpdateChainCoexist 更新成功链：0.9.2 → 0.10.0 双版本共存、列表最新在前、
// 逐版本 Resolve 稳定；卸载新版不动旧版目录。
func TestUpdateChainCoexist(t *testing.T) {
	src := newZipSource(t)
	m := newChainManager(t, src)
	installViaChain(t, m, src, "v0.9.2", "exe-0.9.2")

	rec := &progressRecorder{}
	zipBytes := buildPortableZip(t, "exe-0.10.0")
	seedRemote(t, "v0.10.0", "MarkerOn_0.10.0_x64_portable.zip", int64(len(zipBytes)), shaHex(zipBytes))
	src.set(zipBytes, false)
	if err := m.Download(newTxnID(), "v0.10.0", rec.cb()); err != nil {
		t.Fatalf("Download(v0.10.0): %v", err)
	}
	...
	if installed[0].Version != "v0.10.0" || installed[1].Version != "v0.9.2" {
		t.Errorf("列表应最新在前, got %s, %s", installed[0].Version, installed[1].Version)
	}
	if installed[0].InstalledAt == "" || installed[0].SHA256 == "" {
		t.Errorf("展示账目字段应齐备: %+v", installed[0])
	}
	if want := shaHex([]byte("exe-0.10.0")); installed[0].SHA256 != want {
		t.Errorf("exe 诊断摘要应入账本: got %s want %s", installed[0].SHA256, want)
	}
	...
	exe092, err := m.ResolveExe("v0.9.2")
	if err != nil {
		t.Fatal(err)
	}
	if err := m.Remove("v0.10.0"); err != nil {
		t.Fatalf("Remove(v0.10.0): %v", err)
	}
	if fi, serr := os.Stat(exe092); serr != nil || fi.Size() == 0 {
		t.Error("卸载新版不得动旧版目录")
	}
	if _, err := m.ResolveExe("v0.10.0"); err == nil {
		t.Error("已卸载版本应不可解析")
	}
}
...
func TestReinstallSamePackageIdempotent(t *testing.T) {
	...
	seedRemote(t, "v0.9.2", "MarkerOn_0.9.2_x64_portable.zip", int64(len(zipBytes)), shaHex(zipBytes))
	src.set(zipBytes, false)
	if err := m.Download(newTxnID(), "v0.9.2", nil); err != nil {
		t.Fatalf("首装: %v", err)
	}
	if err := m.Download(newTxnID(), "v0.9.2", nil); err != nil {
	...
	seedRemote(t, "v0.9.2", "MarkerOn_0.9.2_x64_portable.zip", int64(len(tampered)), shaHex(tampered))
	src.set(tampered, false)
	err := m.Download(newTxnID(), "v0.9.2", nil)
