...
func (s *Service) DownloadVersion(targetVersion string) (string, error) {
	...
	go func() {
		...
		if lerr != nil {
			if app := application.Get(); app != nil && app.Event != nil {
				app.Event.Emit("bili23:version-download", version.DownloadProgress{Version: targetVersion, Stage: "error", Message: lerr.Error()})
			}
			notify.Error("bili23", "版本下载失败", fmt.Sprintf("Bili23 Downloader %s 后台租约开启失败: %v", targetVersion, lerr), "/ext/bili23")
			return
		}
		...
		if err := txn.Done(); err != nil {
			emit(version.DownloadProgress{Version: targetVersion, Stage: "error", Message: err.Error()})
			notify.Error("bili23", "版本下载失败", fmt.Sprintf("Bili23 Downloader %s 事务收口失败: %v", targetVersion, err), "/ext/bili23")
			return
		}
...
func (s *Service) ImportLocal(srcDir string) (version.Bili23VersionInfo, error) {
	...
	if err != nil {
		return version.Bili23VersionInfo{}, err
	}
