...
const (
	repoOwner  = "ScottSloan"
	repoName   = "Bili23-Downloader"
	userAgent  = "Hanxi/0.2"
...
// plainSemverTag 纯数字三段版本 tag（格式 vX.Y.Z）。
// 上游存在 v2.00.7 这类前导零变体（asset 名去掉 v 为 2.00.7），语义版本严格解析会
// 判定非法，但这里只做"是否纯数字三段"的结构过滤，允许前导零，防未来非规范 tag 污染列表。
...
// 资产名形如 Bili23-Downloader_2.15.0_windows_x64_portable.zip；同 release 还发布
// installer(.exe)、win7 变体(_for_win7)、Linux AppImage/rpm/deb、macOS dmg——
// 精确名匹配优先，后缀 "_windows_x64_portable.zip" 兜底天然排除其余全部形态。
func findPortableAsset(assets []asset, version string) (asset, bool) {
	ver := strings.TrimPrefix(version, "v")
	needName := strings.ToLower("bili23-downloader_" + ver + "_windows_x64_portable.zip")
	...
	for _, a := range assets {
		...
		if strings.HasSuffix(lower, "_windows_x64_portable.zip") && strings.Contains(lower, strings.ToLower(ver)) {
			return a, true
		}
	}
	return asset{}, false
...
func (c *releaseCache) get() ([]Bili23Release, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// 过滤规则：draft / 非纯数字三段 tag / 无 x64 便携资产 / 官方 sha256 缺失的 release 一律不入列表。
func parseReleasesBody(body []byte) ([]Bili23Release, error) {
	var releases []release
	if err := json.Unmarshal(body, &releases); err != nil {
		return nil, fmt.Errorf("parse releases: %w", err)
	}

	var list []Bili23Release
	for _, r := range releases {
		// 未认证 API 本就不返回 draft，防御性再跳一次；tag 非纯 x.y.z 直接丢弃（含 -rc 预发布）
		...
		list = append(list, Bili23Release{
			Version:   r.TagName,
