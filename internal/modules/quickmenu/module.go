...
//   - 弹窗为常驻隐藏的单例 frameless WebView 窗口，呈圆形轮盘——方形窗口首次
//     唤出后经 GDI 区域裁剪成正圆（beta.10 无窗口透明能力，见 windows.ClipWindowEllipse），
//     失焦收起 + 全局点击按圆判定兜底（前台锁下可能抢不到焦点），
//     多显示器/屏幕边缘经物理↔DIP 换算与工作区钳位。
package quickmenu
