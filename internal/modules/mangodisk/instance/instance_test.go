...
// 本文件聚焦 mangodisk 适配层：内核快照 → 本包 Snapshot 的形状/词表映射、
// Tauri 信号窗 PID 归属探针（external 快照携真实 PID）、ErrExternal 契约映射、
// "已手动停止"文案折回、信使接缝、WM_CLOSE 宽限收口，以及维持既有环境口径的
// 真进程冒烟。
...
type fakeProbe struct {
	...
	signalPID uint32 // SignalPID 返回值（0 = 信号窗口不可得）
}

func (p *fakeProbe) IsRunning() bool                 { return p.running }
func (p *fakeProbe) WaitForReady(time.Duration) bool { return p.ready }
func (p *fakeProbe) SignalPID() (uint32, bool)       { return p.signalPID, p.signalPID != 0 }
...
// 由 Job/句柄负责终止）。个别受限执行环境（如 PATH 精简的沙箱/Git Bash 会话）
// System32 不在 PATH 中：经 SystemRoot 定位兜底，仍是真实系统 cmd.exe，
// 不改变冒烟口径。
...
func TestExitCodeAndMessageMapping(t *testing.T) {
	...
	e.onSupState(sup.Snapshot{State: sup.StateFailed, PID: 0, Version: "v1.0.7",
		Error: "托管进程异常退出（退出码 1）"})
	snap := rec.last()
	if snap.State != StateFailed || snap.ExitCode != 1 {
		t.Fatalf("failed 映射异常: %+v", snap)
	}
	if !strings.Contains(snap.Error, "MangoDisk 异常退出（退出码 1）") ||
		!strings.Contains(snap.Error, "程序文件完整") || !strings.Contains(snap.Error, "WebView2") {
		t.Errorf("异常退出文案应保持 mangodisk 既有口径: %s", snap.Error)
	}
	...
	// external：External 标志与 PID 账目透传（内核携 ProcInfo 时为真实外部 PID）
	...
	e.onSupState(sup.Snapshot{State: sup.StateExternal, PID: 4321})
	if snap = rec.last(); snap.PID != 4321 {
		t.Fatalf("external 快照应如实携带探针 PID: %+v", snap)
	}
}

// ---------- 探针 PID 归属（supervisor.Probe 适配层） ----------

func TestSupProbePIDAttribution(t *testing.T) {
	// 互斥体不在：一律 not running（无归属可言）
	probe := &fakeProbe{running: false, signalPID: 777}
	if running, info, err := (supProbe{probe}).Inspect(nil); running || info != nil || err != nil {
		t.Fatalf("互斥体缺失应判 not running: %v %+v %v", running, info, err)
	}
	// 互斥体在 + 信号窗可得：外部性 + PID 双事实
	probe = &fakeProbe{running: true, signalPID: 4321}
	running, info, err := (supProbe{probe}).Inspect(nil)
	if !running || err != nil || info == nil || info.PID != 4321 {
		t.Fatalf("信号窗 PID 应随 ProcInfo 上报: %v %+v %v", running, info, err)
	}
	// 互斥体在 + 信号窗不可得：仍报存活、无 PID（外部性判定不翻转）
	probe = &fakeProbe{running: true, signalPID: 0}
	running, info, err = (supProbe{probe}).Inspect(nil)
	if !running || err != nil || info != nil {
		t.Fatalf("信号窗缺失应降级为无 PID 的存活: %v %+v %v", running, info, err)
	}
}
...
func TestStartEmptyExeRejected(t *testing.T) {
	...
	if err := e.Start(StartOptions{Version: "v1.0.7"}); err == nil ||
		err.Error() != "MangoDisk EXE 路径不能为空" {
...
func TestStartCreateJobFail(t *testing.T) {
	...
	err := e.Start(StartOptions{Version: "v1.0.7", Exe: mustCmdExe(t)})
	if err == nil {
...
func TestManagedLifecycleSmoke(t *testing.T) {
	...
	exe := mustCmdExe(t)
	if err := e.Start(StartOptions{Version: "v1.0.7", Exe: exe}); err != nil {
...
// 注：冷启动竞速的 wait 退出分类（自有进程自退 + 探针仍见存活 → external 接管）
// 由内核 supervisor 引擎测试矩阵覆盖（fake opener）；本层对"接管快照携探针
// PID"的钉扎见 TestSupProbePIDAttribution 与 TestStopExternalKeepsNilContract。

// TestQuitGraceFallback 冒烟：Quit 经内核 grace 通道——WM_CLOSE（信号窗不可得，
...
func TestQuitGraceFallback(t *testing.T) {
	...
	if err := e.Start(StartOptions{Version: "v1.0.7", Exe: mustCmdExe(t)}); err != nil {
		t.Fatalf("Start: %v", err)
	}
	defer func() { _ = e.Stop() }()
	waitState(t, e, StateRunning)
...
// 按 mangodisk 既有契约映射为成功无操作，状态校正为 external（携探针 PID）。
func TestStopExternalKeepsNilContract(t *testing.T) {
	e := NewEngine(newWindowsJobAPI(), &fakeProbe{running: true, signalPID: 5678}, Callbacks{})
	if err := e.Quit(); err != nil {
		t.Fatalf("external 下 Quit 应映射为 nil, got %v", err)
	}
	if snap := e.Snapshot(); snap.State != StateExternal || !snap.External || snap.PID != 5678 {
...
func TestRefreshExternalTwoWay(t *testing.T) {
	...
	probe.running = true
	probe.signalPID = 4321
	e.RefreshExternal()
	if got := e.Snapshot(); got.State != StateExternal || got.PID != 4321 {
		t.Fatalf("external 校正应入账且携信号窗 PID（mangodisk 探针特判）: %+v", got)
	}
