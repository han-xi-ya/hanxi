// Wave 4-B 安装事务 journal 链路测试（markeron/rufus/ccswitch 同构，资产形态
// 为单便携 exe）：以假下载源驱动 Begin→Advance→Complete 全步、失败收口、
// 强杀模拟（重启 Store 后 Pending 可见）、启动恢复"背书"清理（只删本事务
// staging，无背书孤儿只 Report 不动盘）与单写事务拒并发。全部落真 tmp 目录 +
// 回环假源，无网络依赖。
package version
...
// mangodiskTestSteps 与 MangoDiskService.mangodiskInstallSteps 保持同一词表
// （服务层编排在版本包测试不可达，此处复刻同款阶段映射验证交接面；单 exe 形态
// 无 unpack 步，verify 对应模块侧字节/PE 断言）。
var mangodiskTestSteps = []string{"download", "verify", "place"}
...
func driveInstall(t *testing.T, m *Manager, txnID, version string) error {
	t.Helper()
	txn, err := ops.BeginTxn("mangodisk", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		version, txnID, mangodiskTestSteps)
	if err != nil {
		return err
	}
	stepIdx := -1
...
func TestJournalChainSuccess(t *testing.T) {
	...
	if err := driveInstall(t, m, txnID, "v1.0.7"); err != nil {
	...
	if j.State != operation.TxnSucceeded || j.Operation != operation.TxnOpInstall ||
		j.DeliveryKind != extapi.DeliveryManagedDeclarative || j.ModuleID != "mangodisk" {
		t.Errorf("账本终态异常: %+v", j)
	}
	if j.ToVersion == nil || *j.ToVersion != "v1.0.7" {
...
func TestJournalChainFailureInterrupt(t *testing.T) {
	...
	assertNoTransactionLeftovers(t, m.versionsDir)
	if _, rerr := m.tree.Resolve("1.0.7"); rerr == nil {
		t.Error("失败链不得留下版本目录")
	}
}
...
func TestJournalSingleWriteRejectsConcurrent(t *testing.T) {
	withTestKernel(t)
	_, err := ops.BeginTxn("mangodisk", extapi.OpInstall, extapi.DeliveryManagedDeclarative, "v1.0.6", newTxnID(), mangodiskTestSteps)
	if err != nil {
		t.Fatal(err)
	}
	// 故意不收口：模拟在途/崩溃现场，再开第二笔
	_, err = ops.BeginTxn("mangodisk", extapi.OpUpdate, extapi.DeliveryManagedDeclarative, "v1.0.7", newTxnID(), mangodiskTestSteps)
...
func TestJournalCrashRecoveryEndorsement(t *testing.T) {
	...
	txnID := newTxnID()
	txn, err := ops.BeginTxn("mangodisk", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		"v1.0.7", txnID, mangodiskTestSteps)
	if err != nil {
		t.Fatal(err)
	}
	txn.Step(0)
	...
	_ = discard
	if err := os.WriteFile(filepath.Join(staging, exeName), exeBody("half-killed"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func crashCompensate(store *operation.Store, versionsDir string) operation.Compensation {
	trees := map[string]*artifact.Tree{"mangodisk": OpenTree(versionsDir)}
