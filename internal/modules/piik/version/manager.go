...
const (
	// exeName 托管主入口（zip 根平铺，兄弟路径 runtime/ 双 exe 按 exe 所在
	// 目录相对解析——目录结构不可拆，本包一切布局判定都以此为前提）。
	exeName = "piik-app.exe"

	// revisionFileName 根平铺关键条目之一：REVISION（上游构建标识），
	// 内容读取记账见 readRevision。
	revisionFileName = "REVISION"

	// treeEntryName 版本树目录前缀（<root>/piik_<version>，布局 piik_1.2.0 /
	// piik_imported-<时间戳> 与家族一致：目录名不带 v，展示版本号带 v）。
	treeEntryName = "piik"
	// dirPrefix 导入链直造目录时的版本隔离目录前缀（与 Tree 的 entry 命名同源）。
	dirPrefix = treeEntryName + "_"
	// fetchBudget 单次下载的总超时预算（主 exe 约 46MB + runtime 双件，沿用
	// 家族大包 10 分钟口径）。
	fetchBudget = 10 * time.Minute

	// moduleMetaFileName 模块侧账本（verifiedHash/导入来源/REVISION 记账等
	// 前端契约字段，内核 artifact.Meta 之外的 Piik 领域字段）。
	moduleMetaFileName = "meta.module.json"

	// revisionMaxLedger REVISION 记账截断上限（上游构建标识，防御异常大件）。
	revisionMaxLedger = 200
)

// captureExeRel / cloudflaredExeRel runtime 双 exe 相对路径（孙进程载荷：
// 屏幕采集原生件 + Cloudflare 隧道件）：均相对版本根，缺一即布局残缺、
// 兄弟路径解析失败、运行必炸，故入布局不变式硬判。filepath.Join 非常量
// 表达式，落 var 而非 const。
var (
	captureExeRel     = filepath.Join("runtime", "native", "piik-capture.exe")
	cloudflaredExeRel = filepath.Join("runtime", "tunnel", "cloudflared.exe")
)

// plainVersionRe 纯版本号（如 1.2.0），用于目录名与 FileVersion 校验
...
// Manager Piik 版本管理引擎：远程列表（GitHub 元数据）与"下载 → 校验 →
// 解包 → 落位"主流程委托 Wave 4 共享内核 packages/go/artifact（Fetch +
// UnpackZip + Tree）；本包只保留 Piik 领域知识：裸名便携资产筛选、镜像 URL
// 模板、版本目录形状（x.y.z / imported-时间戳）、平铺自管目录的布局不变式
// 自检（五关键条目在场 + runtime 双 exe 在位——兄弟路径解析令目录结构不可
// 拆，判定比 ccswitch 双锚点、DBX 单标记更宽）、REVISION 记账、账外漂移
// 护栏（落位 sha256 记账 + mtime 闸控复算，照 everything/DBX 实作）、本地
// 整套导入与既有进度词表映射。
type Manager struct {
	versionsDir string
	tree        *artifact.Tree
...
// OpenTree 打开 Piik 版本树（装配根启动恢复按事务背书清理现场时用；
...
func (m *Manager) ListRemote() ([]PiikRelease, error) {
	list, err := remoteCache.get()
	if err != nil {
		return nil, err
	}
	out := make([]PiikRelease, len(list))
	copy(out, list)
	for i := range out {
...
// 目录命名 piik_1.2.0（下载/导入的正规版本）或 piik_imported-YYYYMMDD-HHMMSS
// （版本探测失败的导入兜底）；其余形状的版本令牌不列入；布局不变式
// （主 exe 非空 + runtime 双 exe 在位）不满足视为损坏安装跳过。
// 账外漂移护栏：对每个版本做 mtime 闸控的摘要复查（仅主 exe 落位后有改动
// 迹象才复算全量哈希，成本纪律见 driftCheck 注释），漂移如实投影到
// HashDrifted/DriftNote——不自动处置，不撒谎。
func (m *Manager) ListInstalled() ([]PiikVersionInfo, error) {
	vers, err := m.tree.Versions()
	if err != nil {
		return nil, err
	}

	var list []PiikVersionInfo
	for _, v := range vers {
		...
		// runtime 双 exe 与平铺根同在是兄弟路径解析的激活条件：缺失即布局
		// 残缺（采集/隧道件按 exe 目录相对定位），运行必炸，视为损坏跳过
		if _, err := os.Stat(filepath.Join(v.Dir, captureExeRel)); err != nil {
			continue
		}
		if _, err := os.Stat(filepath.Join(v.Dir, cloudflaredExeRel)); err != nil {
			continue
		}

		mm := readModuleMeta(v.Dir)
		info := PiikVersionInfo{
			Version:      version,
			ExePath:      exe,
			Dir:          v.Dir,
			...
			Source:       mm.Source,
			Revision:     revisionForDisplay(v.Dir, mm),
		}
		// 新下载链与导入链都经内核统一账本（artifact.Meta，含 schema 与
		// InstalledAt）；账本缺失的历史/异端目录回退主 exe mtime 展示。
...
// Download 下载便携 zip 并安装到 versions/piik_X.Y.Z/（旧调用面，
// 供版本包单测与非事务调用使用）。
...
// （digest，Piik 的唯一信任根——.sha256 sidecar 自 v1.2.0 停发不依赖）为基准
// 做流式 + 落盘双 SHA-256 校验，Content-Length 与流式上限双核（对齐"字节数
// == release 声明 size"层），镜像只是同摘要的备用传输来源；解包经
// artifact.UnpackZip（ZipSlip/炸弹/CRC32 全量闸门）进独占中转目录，平铺
// 自管目录解包——条目直接落 versions/piik_<ver>/ 根（上游 zip 根平铺，
// 无包裹层，Tree.Commit 原子 rename 收口）；布局不变式不满足即丢弃 staging
// 拒装；落位经 Tree.Commit（同版本异摘要防漂移）。
...
func (m *Manager) DownloadContext(ctx context.Context, txnID, version string, onProgress func(p DownloadProgress)) error {
	...
	// 并入列表，无摘要的 release 根本进不来）。缺失一律拒装，不做无校验安装。
	if rel.SHA256 == "" {
		err := fmt.Errorf("上游未提供 Piik %s 的官方 SHA-256 摘要，拒绝无校验安装", version)
		emit("error", 0, 0, err.Error())
		return err
	}

	tmpZip, err := os.CreateTemp("", "hanxi-piik-*.zip")
	if err != nil {
		return err
	}
	...
	// 4. 平铺布局不变式自检（模块策略，内核不感知）：五关键条目在场 +
	// runtime 双 exe 在位。不符即判定安装无效，调用方丢弃 staging。
	if err := checkPiikLayout(staging); err != nil {
		emit("error", 0, 0, err.Error())
		return err
	}
	// 主 exe 实测摘要：账外漂移护栏的比对锚点（避免每次扫描现场哈希）
	assetSHA, err := fileSHA256(filepath.Join(staging, exeName))
	if err != nil {
		emit("error", 0, 0, fmt.Sprintf("计算落位摘要失败: %v", err))
		return err
	}
	// REVISION 记账 + 模块侧账本先写进 staging（随目录原子落位，杜绝
	// "半件有内容无账"窗口）
...
// ResolveExe 返回指定版本 piik-app.exe 的绝对路径（不存在返回错误）
...
// PEVersion 返回版本主 exe 的 PE 文件版本（供前端展示与导入查重旁证；
// 读不到返回空串）。
...
// VerifyLedger 账外漂移护栏只读复查（照 DBX 实作）：将指定版本主 exe 的
...
// 才做 ~46MB 全量哈希，mtime 未变直接判"无漂移迹象"——旁证性护栏，不是
...
// DBX/gonavi 同纪律）：账本无落位摘要（历史/异端安装）→ 如实报"无法比对"，
...
func driftCheck(exe string, fi os.FileInfo, meta artifact.Meta) (bool, string) {
	...
	if !fi.ModTime().After(landed) {
		return false, "复算未触发：主 exe 自落位后无改动迹象"
	}
...
// resolveVersionDir 定位版本隔离目录（piik_X.Y.Z 或 piik_imported-时间戳）。
...
// （目录名不带 v、展示带 v，与 ccswitch/DBX 同口径）。仅接受纯 x.y.z 或
// imported-时间戳形状，piik_v1.2.0 之类带 v 前缀的历史/外来目录名不列入。
...
// checkPiikLayout 平铺布局不变式自检（模块策略，拒装面按契约钉死）：
// 根平铺五关键条目在场（piik-app.exe / LICENSE / REVISION / NOTICES /
// runtime 目录）且 runtime 双 exe 在位（native/piik-capture.exe +
// tunnel/cloudflared.exe，均非空普通文件）。兄弟路径 exe 按目录相对解析，
// 目录结构不可拆——runtime 双件缺一即运行必炸，主 exe 空即入口损坏。
// 与 DBX"许可文本只看不拒"不同：Piik 上游 19 连版实测条目名稳定，许可
// 文本族（MIT 随附义务）与 REVISION（漂移取证锚）一并入不变式；若未来
// 上游改名条目导致拒装，属 fail-closed 的正确姿态，由人工裁决而非静默放行。
// 不符即判定安装无效，调用方丢弃 staging。
func checkPiikLayout(dir string) error {
	...
	for _, name := range []string{"LICENSE", revisionFileName, "NOTICES"} {
		s, serr := os.Stat(filepath.Join(dir, name))
		if serr != nil || s.IsDir() {
			return fmt.Errorf("zip 布局无效：根平铺关键条目 %s 不在场", name)
		}
	}
	if d, derr := os.Stat(filepath.Join(dir, "runtime")); derr != nil || !d.IsDir() {
		return fmt.Errorf("zip 布局无效：runtime 目录不在场（兄弟路径解析依赖整目录结构）")
	}
	for _, rel := range []string{captureExeRel, cloudflaredExeRel} {
		s, serr := os.Stat(filepath.Join(dir, rel))
		if serr != nil || s.IsDir() || !s.Mode().IsRegular() || s.Size() == 0 {
			return fmt.Errorf("zip 布局无效：runtime 双 exe 缺位（%s）", rel)
		}
	}
	return nil
}

// readRevision 读取 staging/落位目录内 REVISION 内容（上游构建标识），
// 供账本记账与漂移取证对照；异常/超大截断如实降级为空串或截断值。
func readRevision(dir string) string {
	raw, err := os.ReadFile(filepath.Join(dir, revisionFileName))
	if err != nil {
		return ""
	}
	rev := strings.TrimSpace(string(raw))
	if len(rev) > revisionMaxLedger {
		rev = rev[:revisionMaxLedger]
	}
	return rev
}

// revisionForDisplay REVISION 展示值：账本记账优先，历史/异端安装回退
// 现场读取（小文件，扫描期读取成本可忽略）。
func revisionForDisplay(dir string, mm moduleMeta) string {
	if mm.Revision != "" {
		return mm.Revision
	}
...
// moduleMeta 模块侧账本（内核 artifact.Meta 之外的 Piik 领域字段）。
type moduleMeta struct {
	IsImport     bool   `json:"isImport,omitempty"`
	Source       string `json:"source,omitempty"`
	Copied       string `json:"copied,omitempty"`
	VerifiedHash bool   `json:"verifiedHash,omitempty"`
	Revision     string `json:"revision,omitempty"` // REVISION 落位时记账
}
...
// dirSize 版本目录整树字节和：主程序文件只是入口，runtime 双 exe 才是体量
// 的主体，单报主 exe 尺寸与真实安装体量级失真。dirstats 度量恒跳过符号链接/
...
func dirSize(dir string, fallback int64) int64 {
	...
	if st.Err != nil || st.Partial || st.Bytes <= 0 {
		slog.Debug("piik 版本目录大小度量降级，回退主程序文件大小", "dir", dir, "partial", st.Partial, "err", st.Err)
		return fallback
	}
	return st.Bytes
}
