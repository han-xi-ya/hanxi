...
// 说明：进程治理主流程由内核 supervisor 测试矩阵覆盖。本文件聚焦 termora
...
func (fp *fakeProbe) MutexHeld() bool                 { return true }
...
func TestMapKernelStateVocabulary(t *testing.T) {
	...
	// 窗口外异常退出：Termora 文案（引导回版本管理重装）
	e.onSupState(sup.Snapshot{State: sup.StateFailed, Error: "托管进程异常退出（退出码 2）"})
	if got := rec.last(); got.State != StateFailed || !strings.Contains(got.Error, "Termora 异常退出（退出码 2）") {
...
func TestStoppedMessageAndPidShape(t *testing.T) {
	...
	e.onSupState(sup.Snapshot{State: sup.StateRunning, Version: "v2.7.0", PID: 4242, Exe: `C:\v\Termora.exe`, Since: time.Now()})
	run := rec.last()
	if run.PID != 4242 || run.ExePath != `C:\v\Termora.exe` || run.Version != "v2.7.0" {
		t.Fatalf("running 快照形状异常: %+v", run)
	}
	e.onSupState(sup.Snapshot{State: sup.StateStopped, Version: "v2.7.0", PID: 4242, Exe: `C:\v\Termora.exe`, Error: "已手动停止"})
	stop := rec.last()
...
func TestFinishQuitAttributionTable(t *testing.T) {
	...
	snap := sup.Snapshot{State: sup.StateRunning, PID: 42, Exe: `C:\v\Termora.exe`}
...
func TestVerifyTokenRejectsPathMismatch(t *testing.T) {
	...
	err := e.verifyToken(platform.VerifyToken{PID: 10, ExePath: filepath.Join(dir, "Termora.exe"), StartedAt: proc.info.StartedAt})
...
func TestMessengerSpawnFireAndForget(t *testing.T) {
	e := NewEngine(&fakeJobAPI{}, &fakeProcessAPI{}, &fakeProbe{}, Callbacks{})
	var got string
	e.launchMessenger = func(exe string) error { got = exe; return nil }
	if err := e.Messenger(`C:\Termora.exe`); err != nil {
		t.Fatal(err)
	}
	if got != `C:\Termora.exe` {
		t.Fatalf("信使应原样拉起目标 exe: %q", got)
	}
	// 信使不经内核：不产生任何状态迁移账（内核仍静止）
	if st := e.Snapshot().State; st != StateStopped {
		t.Fatalf("信使不得扰动引擎状态机: %s", st)
	}
}
...
func TestQuitExternalDeclinedKeepsExternal(t *testing.T) {
	...
	info := platform.ProcInfo{PID: 9001, ExePath: `C:\ext\Termora.exe`, StartedAt: time.Now()}
...
func TestQuitExternalForcedAfterConsent(t *testing.T) {
	...
	info := platform.ProcInfo{PID: 9002, ExePath: `C:\ext\Termora.exe`, StartedAt: time.Now()}
	proc := &fakeProcessAPI{info: info}
...
func TestQuitExternalBlockedIsHonest(t *testing.T) {
	...
	info := platform.ProcInfo{PID: 9003, ExePath: `C:\ext\Termora.exe`, StartedAt: time.Now()}
...
func TestStartRejectsEmptyExe(t *testing.T) {
	e := NewEngine(&fakeJobAPI{}, &fakeProcessAPI{}, &fakeProbe{}, Callbacks{})
	if err := e.Start(StartOptions{Version: "v2.7.0"}); err == nil || err.Error() != "Termora.exe 路径不能为空" {
