...
type BCUService struct {
	...
	downloadDriver func(ctx context.Context, txnID, targetVersion, variant string, emit func(version.DownloadProgress)) error
...
func (s *BCUService) DownloadVersion(targetVersion, variant string) (string, error) {
	...
	go func() {
		...
		if err := drive(txn.Context(), txnID, targetVersion, variant, emit); err != nil {
			// N26 用户主动取消：按 2b 纪律如实收口，票面话术不露 ctx 原始错误；
			// 主动动作不发"失败"系统通知（前端票面已呈现「已取消」）。
			if txn.Err() != nil {
				txn.Fail("operation-cancelled", "托管操作已取消")
				emit(version.DownloadProgress{Version: targetVersion, Variant: variant, Stage: "error", Message: "托管下载已取消"})
			} else {
