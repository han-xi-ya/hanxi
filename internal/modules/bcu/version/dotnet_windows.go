...
// HasDesktopRuntimeMajor 已装桌面运行时中是否存在主版本为 major 的（如 8 → 8.0.x）。
// .NET 运行时不跨大版本回退：8.0 目标程序需要 8.x 运行时，装了 9.x 也跑不起来，
// 所以推荐判断必须按主版本匹配。
