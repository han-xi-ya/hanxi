...
// TestRemoveRefusedWhileExeLocked 文件锁拒卸：版本目录内 win-x64 真身被非共享
// 句柄占用（模拟"正在运行"对文件系统的锁效应——托管启动锚点即内层，锁也应
// 打在内层）时，Tree.Remove 的 rename 隔离必须失败并给出人话错误，目录保持
// 原样；解锁后方可卸载。
func TestRemoveRefusedWhileExeLocked(t *testing.T) {
	src := newZipSource(t)
	m := newChainManager(t, src)
	installViaChain(t, m, src, "6.2.0", "locked-exe")

	exePath := filepath.Join(m.versionsDir, "bcu_6.2.0", "win-x64", exeName)
	...
	if rmErr := m.Remove("6.2.0"); rmErr == nil {
