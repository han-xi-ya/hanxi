...
// bcuTestSteps 与 BCUService.bcuInstallSteps 同一词表（服务层编排在本包不可达，
// 此处复刻同款阶段映射以验证交接面；verify 由内核 Fetch 折进 download 步，
// 不造幻影步骤）。
var bcuTestSteps = []string{"download", "unpack", "place"}
...
// driveInstall 复刻 service.DownloadVersion 后台协程的事务编排（bcu 固定走
// 自包含便携变体；fdd 分叉由 Download 自身用例覆盖）。
func driveInstall(t *testing.T, m *Manager, txnID, version string) error {
	t.Helper()
	txn, err := ops.BeginTxn("bcu", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		version, txnID, bcuTestSteps)
	if err != nil {
		return err
	}
	stepIdx := -1
	...
	if err := m.Download(txnID, version, VariantPortable, emit); err != nil {
...
func TestJournalChainSuccess(t *testing.T) {
	...
	if err := driveInstall(t, m, txnID, "6.2.0"); err != nil {
	...
	if j.State != operation.TxnSucceeded || j.Operation != operation.TxnOpInstall ||
		j.DeliveryKind != extapi.DeliveryManagedDeclarative || j.ModuleID != "bcu" {
		t.Errorf("账本终态异常: %+v", j)
	}
...
func TestJournalChainFailureInterrupt(t *testing.T) {
	...
	src := newZipSource(t)
	m := newChainManager(t, src)
	installViaChain(t, m, src, "6.1.0.1", "old-exe")
...
func TestJournalSingleWriteRejectsConcurrent(t *testing.T) {
	withTestKernel(t)
	if _, err := ops.BeginTxn("bcu", extapi.OpInstall, extapi.DeliveryManagedDeclarative, "6.1.0.1", newTxnID(), bcuTestSteps); err != nil {
		t.Fatal(err)
	}
	_, err := ops.BeginTxn("bcu", extapi.OpUpdate, extapi.DeliveryManagedDeclarative, "6.2.0", newTxnID(), bcuTestSteps)
...
func TestJournalCrashRecoveryEndorsement(t *testing.T) {
	...
	trees := map[string]*artifact.Tree{"bcu": OpenTree(m.versionsDir)}
