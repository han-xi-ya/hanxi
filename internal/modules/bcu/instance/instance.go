...
//   - 已有主实例（自有/外部）→ 无参拉起"信使"进程，BCU 第二实例
//     查找主进程后 SetForegroundWindow 唤起主窗口；
...
//  1. EnumWindows 按 PID 向全部顶层窗口投递 WM_CLOSE——BCU 的 FormClosing
//     走正常关闭路径（设置落盘、互斥体释放、退出码 0）；
//  2. 宽限 closeGracePeriod 轮询进程自然退出；
//  3. 超时（对话框挂起/主窗口未创建等）JobObject 强杀兜底。
...
func (e *Engine) wait() {
	...
	case code == 0 && prev == StateRunning:
		e.transition(StateStopped, "") // 用户在 BCU 自己退出
	default:
		e.transition(StateFailed, fmt.Sprintf("BCU 异常退出（退出码 %d）。请尝试重新安装该版本", code))
	}
}
