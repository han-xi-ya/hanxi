...
// 位置 <stateDir>/bcu.json，仅存 activeVersion（空字符串 = 未指定，冷启动自动回退最新已装）。
...
func (s *bcuStore) saveLocked() error {
	return jsonstore.Save(s.filePath, bcuConfig{ActiveVersion: s.activeVersion, FollowOnExit: &s.followOnExit})
}
