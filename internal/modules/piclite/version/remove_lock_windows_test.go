...
// TestRemoveRefusedWhileExeLocked 文件锁拒卸：版本目录内 piclite.exe 被非共享
...
func TestRemoveRefusedWhileExeLocked(t *testing.T) {
	...
	installViaChain(t, m, src, "v1.4.0", "locked-exe")

	exePath := filepath.Join(m.versionsDir, "piclite_1.4.0", exeName)
	...
	if rmErr := m.Remove("v1.4.0"); rmErr == nil {
	...
	if err := m.Remove("v1.4.0"); err != nil {
