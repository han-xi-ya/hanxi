...
// MSI 管理提取）：假下载源 + 假提取接缝驱动 Begin→Advance→Complete 全步、
// 中断失败收口、强杀模拟（重启 Store 后 Pending 可见）与启动恢复"背书"清理。
// 全部真 tmp 目录 + 回环假源，无网络依赖、不触本机 Installer 服务。
package version
...
// picliteTestSteps 与 PicLiteService.picliteInstallSteps 同一词表（服务层
// 编排在本包不可达，此处复刻同款阶段映射以验证交接面；MSI 形态下 verify
// 由内核 Fetch 折进 download 步，unpack 步对应 msiexec /a 管理提取+布局收割）。
var picliteTestSteps = []string{"download", "unpack", "place"}
...
func driveInstall(t *testing.T, m *Manager, txnID, version string) error {
	t.Helper()
	txn, err := ops.BeginTxn("piclite", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		version, txnID, picliteTestSteps)
	if err != nil {
		return err
	}
	stepIdx := -1
...
func TestJournalChainSuccess(t *testing.T) {
	...
	if err := driveInstall(t, m, txnID, "v1.4.1"); err != nil {
	...
	if j.State != operation.TxnSucceeded || j.Operation != operation.TxnOpInstall ||
		j.DeliveryKind != extapi.DeliveryManagedDeclarative || j.ModuleID != "piclite" {
		t.Errorf("账本终态异常: %+v", j)
	}
...
func TestJournalChainFailureInterrupt(t *testing.T) {
	...
	src := newMSISource(t)
	m := newChainManager(t, src)
	installViaChain(t, m, src, "v1.4.0", "old-exe")

	txnID := newTxnID()
	newBytes := fakeMSI("interrupted")
...
func TestJournalSingleWriteRejectsConcurrent(t *testing.T) {
	withTestKernel(t)
	if _, err := ops.BeginTxn("piclite", extapi.OpInstall, extapi.DeliveryManagedDeclarative, "v1.4.0", newTxnID(), picliteTestSteps); err != nil {
		t.Fatal(err)
	}
	_, err := ops.BeginTxn("piclite", extapi.OpUpdate, extapi.DeliveryManagedDeclarative, "v1.4.1", newTxnID(), picliteTestSteps)
...
func TestJournalCrashRecoveryEndorsement(t *testing.T) {
	...
	trees := map[string]*artifact.Tree{"piclite": OpenTree(m.versionsDir)}
