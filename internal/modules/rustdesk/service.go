...
// RustDeskService 向前端暴露 RustDesk 版本管理与启停/唤窗能力。
...
func (s *RustDeskService) SetActiveVersion(targetVersion string) (string, error) {
	targetVersion = "v" + strings.TrimPrefix(strings.TrimSpace(targetVersion), "v")
...
func (s *RustDeskService) resolveActiveVersion() (string, string, error) {
	...
	if len(installed) == 0 {
		return "", "", fmt.Errorf("尚未安装任何 RustDesk 版本，请先在版本管理下载或导入")
	}
