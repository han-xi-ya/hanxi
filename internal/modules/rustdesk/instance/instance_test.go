...
func startEngine(t *testing.T, api platform.JobAPI, probe RustDeskProbe, rec Callbacks) *Engine {
	t.Helper()
	e := NewEngine(api, probe, rec)
	if err := e.Start(StartOptions{Version: "v1.4.9", Exe: mustCmdExe(t)}); err != nil {
...
func TestStartPromoteRunningThenTreeGoneStopped(t *testing.T) {
	...
	if snap := e.Snapshot(); snap.PID != 111 || snap.Version != "v1.4.9" {
