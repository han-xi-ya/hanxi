...
//
// 进程身份双来源（两形态纳管后）：便携提取目录（%LOCALAPPDATA%\rustdesk）
// ∪ 安装版目录（注册表探测，如 C:\Program Files\RustDesk）。SYSTEM 服务与
// 服务派生 broker 因镜像路径不可读被天然排除（非提权 OpenProcess 失败），
// "服务常驻被误判为实例运行"由 Windows 权限边界兜底、代码层不赌运气：
// 只统计"路径可读且命中目录前缀"的进程。
type RustDeskProbe interface {
	...
	// ancestors 任一成员的进程 PID。安装版直拉主进程时祖先本体即实例，
	// 须计入（packer 外层镜像在托管目录、不命中前缀，天然不受影响）。
