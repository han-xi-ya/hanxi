...
// plainSemverTag 严格纯语义版本 tag（格式 X.Y.Z）：上游 tag 不带 v 前缀
// （1.4.9 等），nightly 等非规范 tag 直接丢弃，防其污染版本列表。
...
// 资产名形如 rustdesk-1.3.0-x86_64.exe；同 release 还含 aarch64.exe / msi /
...
func findPortableAsset(assets []asset, version string) (asset, bool) {
	...
	needName := strings.ToLower("rustdesk-" + ver + "-x86_64.exe")
...
// findInstallerAsset 筛选 Windows x64 安装版 MSI（rustdesk-1.3.0-x86_64.msi，
// WiX perMachine 包，真机实证与便携 exe 成对发布）。官方 digest 缺失的候选
// 视为不可用（完整性第一层校验不能缺位，与便携同纪律）。
func findInstallerAsset(assets []asset, version string) (asset, string, bool) {
	ver := strings.TrimPrefix(version, "v")
	needName := strings.ToLower("rustdesk-" + ver + "-x86_64.msi")
	var fallback asset
...
func (c *releaseCache) get() ([]RDRelease, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func parseReleasesBody(body []byte) ([]RDRelease, error) {
	...
	for _, r := range releases {
		...
		item := RDRelease{
			// tag 无 v 前缀 → 显示/目录统一规范成 vX.Y.Z，Tag 字段保留原始值供下载 URL 构造
			Version:   "v" + r.TagName,
			Tag:       r.TagName,
