...
const (
	exeName   = "rustdesk.exe" // packer 外层落盘定名（恰与内层条目同名）：ResolveExe 确定性寻径；提取目录由内层条目名派生（%LOCALAPPDATA%\rustdesk）
	dirPrefix = "rustdesk_"    // 版本隔离目录前缀（与 ccswitch_ / frpc_v 同构）
)
...
// dirNameRe 版本目录名（rustdesk_1.3.0）；imported- 分支收纳探测失败的时间戳兜底
var dirNameRe = regexp.MustCompile(`^` + dirPrefix + `(?:[0-9][0-9a-zA-Z.]+|imported-\d{8}-\d{6})$`)

// assetVerRe 官方资产文件名中的版本号段（rustdesk-1.3.0-x86_64.exe → 1.3.0）
var assetVerRe = regexp.MustCompile(`(?i)^rustdesk-?v?(\d+\.\d+\.\d+)`)

// importCandidateRe 导入候选：官方形态便携 exe；msi 安装器与 sciter/aarch64 变体绝不可收
var importCandidateRe = regexp.MustCompile(`(?i)^rustdesk-.*\.exe$`)

// Manager RustDesk 版本管理引擎：远程列表、单文件下载校验、隔离目录与本地导入。
type Manager struct {
	versionsDir string
...
func (m *Manager) ListRemote() ([]RDRelease, error) {
	return remoteCache.get()
}

// ListInstalled 扫描本地已安装版本目录。exe 缺失/为空视为损坏安装跳过。
func (m *Manager) ListInstalled() ([]RDVersionInfo, error) {
...
func (m *Manager) resolveRelease(version string) (*RDRelease, error) {
...
// Download 下载便携 packer exe 安装到 versions/rustdesk_X.Y.Z/rustdesk.exe。
// 单文件无 zip 布局，完整性校验：
...
func (m *Manager) Download(version string, onProgress func(p DownloadProgress)) error {
	...
	tmpFile, err := os.CreateTemp("", "hanxi-rustdesk-*.exe")
	if err != nil {
		return err
	}
...
// resolveVersionDir 定位版本隔离目录（rustdesk_X.Y.Z 或 rustdesk_imported-时间戳）
...
func (m *Manager) ImportLocal(path string) (RDVersionInfo, error) {
	...
	if _, err := os.Stat(targetDir); err == nil {
		return RDVersionInfo{}, fmt.Errorf("版本 v%s 已安装，请先卸载再导入", version)
	}
	if err := os.MkdirAll(targetDir, 0755); err != nil {
		return RDVersionInfo{}, err
	}
...
// 独立于版本隔离目录：ListInstalled 按 rustdesk_ 前缀扫描，混放会被损坏判定
// 逻辑误伤；且"下载成功但用户未点安装/中途取消"的包需留存供重试免重下。
func (m *Manager) installerCacheDir() string {
	return filepath.Join(filepath.Dir(m.versionsDir), "installers", "rustdesk")
}

// InstallerCached 返回指定版本已缓存且完好（字节数+官方哈希+MSI 魔数）的安装包
// 路径；"" = 未下载或缓存已损坏（需重新下载）。
func (m *Manager) InstallerCached(rel *RDRelease) string {
...
func (m *Manager) DownloadInstaller(version string, onProgress func(p DownloadProgress)) error {
	...
	if err := downloadTo(m.client, assetMirrors(rel.Tag, rel.InstallerName), tmpPath, func(done int64) {
...
// 真机实证形态为四段"X.Y.Z.构建修订号"（如 1.4.9.29722256）；
...
func (m *Manager) SystemVersion() (RDVersionInfo, bool) {
	si, ok := DetectSystemInstall()
	if !ok {
		return RDVersionInfo{}, false
	}
...
func resolveImportExe(path string) (string, error) {
	...
	if !fi.IsDir() {
		...
		if !importCandidateRe.MatchString(fi.Name()) {
			return "", fmt.Errorf("文件名不符合官方便携版形态（rustdesk-版本-x86_64.exe）: %s", fi.Name())
		}
	...
	if best == "" {
		return "", fmt.Errorf("目录中未找到官方形态的便携 exe（rustdesk-*-x86_64.exe）: %s", path)
	}
