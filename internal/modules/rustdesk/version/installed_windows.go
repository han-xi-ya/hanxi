...
//   - 主程序名：组件 File 元素 Name=$(var.Product).exe → RustDesk.exe，
//     首字母大写，与便携 packer 定名 rustdesk.exe 大小写互补（EqualFold
//     比较处需留意语义选择）；
//   - DisplayVersion：四段"版本.构建修订号"（如 1.4.9.29722256），前
//     三段才是发布版本。
const (
	systemDisplayName = "RustDesk"
	installedExeName  = "RustDesk.exe"
)

// DetectSystemInstall 探测系统级安装的 RustDesk（MSI perMachine → Program Files）。
