...
// 位置 <dataDir>/rustdesk.json，仅存 activeVersion（空字符串 = 未指定，冷启动自动回退最新已装）
...
func (s *rustdeskStore) saveLocked() error {
	...
	bytes, err := json.MarshalIndent(rustdeskConfig{ActiveVersion: s.activeVersion, FollowOnExit: &s.followOnExit}, "", "  ")
	if err != nil {
		return err
	}
...
func (s *rustdeskStore) GetActive() string {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *rustdeskStore) SetActive(version string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
