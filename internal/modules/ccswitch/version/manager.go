...
const (
	...
	// treeEntryName 版本树目录前缀（<root>/ccswitch_<version>，与历史布局
	// ccswitch_3.20.0 / ccswitch_imported-<时间戳> 一致）。
	treeEntryName = "ccswitch"
...
// Manager CC Switch 版本管理引擎：远程列表（GitHub 元数据）与"下载 → 校验 →
// 解包 → 落位"主流程委托 Wave 4 共享内核 packages/go/artifact（Fetch +
// UnpackZip + Tree）；本包只保留 CC Switch 领域知识：便携资产筛选、镜像 URL
// 模板、版本目录形状（x.y.z / imported-时间戳）、绿色版锚点（exe +
// portable.ini）布局自检、本地导入白名单与既有进度词表映射。
type Manager struct {
	versionsDir string
	tree        *artifact.Tree
...
// OpenTree 打开 CC Switch 版本树（装配根启动恢复按事务背书清理现场时用；
...
// 目录命名 ccswitch_X.Y.Z（下载/导入的正规版本）或
// ccswitch_imported-YYYYMMDD-HHMMSS（版本探测失败的导入兜底）；
// 其余形状的版本令牌不列入；exe 缺失/为空或便携标记缺失均视为损坏安装跳过。
...
// Download 下载便携 zip 并安装到 versions/ccswitch_X.Y.Z/。
// 完整性主流程收口至内核 artifact.Fetch：以 GitHub API 官方资产摘要（digest，
// 已解析进 CCRelease.SHA256）为信任根做流式 + 落盘双 SHA-256 校验，
...
func (m *Manager) DownloadContext(ctx context.Context, txnID, version string, onProgress func(p DownloadProgress)) error {
	...
	// 已剥前缀入 CCRelease.SHA256，无摘要的 release 根本不进列表）。缺失一律
	// 拒装，不做无校验安装。
	if rel.SHA256 == "" {
		err := fmt.Errorf("上游未提供 CC Switch %s 的官方 SHA-256 摘要，拒绝无校验安装", version)
		emit("error", 0, 0, err.Error())
		return err
	}
	...
	// 4. 绿色版锚点布局自检（模块策略，内核不感知）：exe 非空 + portable.ini 存在
	if err := checkPortableLayout(staging); err != nil {
		emit("error", 0, 0, err.Error())
		return err
	}
	// exe 诊断摘要入账（避免每次扫描现场哈希；CCVersionInfo 不展示，纯账本诊断）
...
// （3.20.0 → v3.20.0，与原目录名剥离前缀的口径一致）。仅接受纯 x.y.z 或
// imported-时间戳形状，ccswitch_vX.Y.Z 之类带 v 前缀的历史/外来目录名不列入。
...
// checkPortableLayout 绿色版锚点自检（模块策略）：staging 内 cc-switch.exe
// 存在且非空、portable.ini 标记存在。不符即判定安装无效，调用方丢弃 staging。
...
func dirSize(dir string, fallback int64) int64 {
	...
	if st.Err != nil || st.Partial || st.Bytes <= 0 {
		slog.Debug("ccswitch 版本目录大小度量降级，回退主程序文件大小", "dir", dir, "partial", st.Partial, "err", st.Err)
		return fallback
	}
	return st.Bytes
}
