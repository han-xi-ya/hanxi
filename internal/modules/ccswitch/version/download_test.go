...
// 验证中断、坏摘要、半件残留、双版本共存等收口行为（markeron 同构黄金样本）。
package version
...
func newChainManager(t *testing.T, src *zipSource) *Manager {
	...
	m.mirrors = func(_, _ string) []string {
		return []string{src.srv.URL + "/CC-Switch-Windows-Portable.zip"}
	}
	return m
}

// buildPortableZip 官方便携 zip 形状：exe + portable.ini 绿色版锚点 + 附带文件。
...
func installViaChain(t *testing.T, m *Manager, src *zipSource, version, exeContent string) {
	...
	seedRemote(t, version, "CC-Switch-"+strings.TrimPrefix(version, "v")+"-Windows-Portable.zip",
...
func TestDownloadInterruptKeepsOldVersion(t *testing.T) {
	...
	installViaChain(t, m, src, "v3.19.0", "old-exe-bytes")

	newZip := buildPortableZip(t, "brand-new-exe")
	seedRemote(t, "v3.20.0", "CC-Switch-v3.20.0-Windows-Portable.zip", int64(len(newZip)), shaHex(newZip))
	src.set(newZip, true) // 半路掐线

	rec := &progressRecorder{}
	err := m.Download(newTxnID(), "v3.20.0", rec.cb())
	if err == nil {
		t.Fatal("下载中断应报错")
	}
	stages := rec.stages()
	...
	if _, statErr := os.Stat(filepath.Join(m.versionsDir, "ccswitch_3.20.0")); !os.IsNotExist(statErr) {
		t.Error("新半件目录不得出现")
	}
	assertNoTransactionLeftovers(t, m.versionsDir)
	installed, lerr := m.ListInstalled()
	if lerr != nil || len(installed) != 1 || installed[0].Version != "v3.19.0" {
...
func TestDownloadBadDigestRejects(t *testing.T) {
	...
	m := newChainManager(t, src)
	installViaChain(t, m, src, "v3.19.0", "old-exe-bytes")
	oldExeHashBefore, _ := fileSHA256(filepath.Join(m.versionsDir, "ccswitch_3.19.0", exeName))

	newZip := buildPortableZip(t, "tampered-payload")
	seedRemote(t, "v3.20.0", "CC-Switch-v3.20.0-Windows-Portable.zip", int64(len(newZip)),
		shaHex([]byte("NOT-THE-REAL-DIGEST"))) // 官方摘要对不上任何传输字节
	src.set(newZip, false)

	err := m.Download(newTxnID(), "v3.20.0", nil)
	if err == nil {
		t.Fatal("摘要不符应拒收")
	}
	...
	assertNoTransactionLeftovers(t, m.versionsDir)
	oldExeHashAfter, _ := fileSHA256(filepath.Join(m.versionsDir, "ccswitch_3.19.0", exeName))
...
func TestDownloadMissingDigestRefusesUnverified(t *testing.T) {
	...
	seedRemote(t, "v3.20.0", "CC-Switch-v3.20.0-Windows-Portable.zip", int64(len(zipBytes)), "")
	src.set(zipBytes, false)
...
// TestUpdateChainCoexist 更新成功链：3.19.0 → 3.20.0 双版本共存、列表最新在前、
// 逐版本 Resolve 稳定；卸载新版不动旧版目录。
func TestUpdateChainCoexist(t *testing.T) {
	src := newZipSource(t)
	m := newChainManager(t, src)
	installViaChain(t, m, src, "v3.19.0", "exe-3.19.0")

	rec := &progressRecorder{}
	zipBytes := buildPortableZip(t, "exe-3.20.0")
	seedRemote(t, "v3.20.0", "CC-Switch-v3.20.0-Windows-Portable.zip", int64(len(zipBytes)), shaHex(zipBytes))
	src.set(zipBytes, false)
	if err := m.Download(newTxnID(), "v3.20.0", rec.cb()); err != nil {
		t.Fatalf("Download(v3.20.0): %v", err)
	}
	...
	if installed[0].Version != "v3.20.0" || installed[1].Version != "v3.19.0" {
	...
	exe3190, err := m.ResolveExe("v3.19.0")
	if err != nil {
		t.Fatal(err)
	}
	if err := m.Remove("v3.20.0"); err != nil {
		t.Fatalf("Remove(v3.20.0): %v", err)
	}
	if fi, serr := os.Stat(exe3190); serr != nil || fi.Size() == 0 {
		t.Error("卸载新版不得动旧版目录")
	}
	if _, err := m.ResolveExe("v3.20.0"); err == nil {
		t.Error("已卸载版本应不可解析")
	}
}
...
func TestReinstallSamePackageIdempotent(t *testing.T) {
	...
	seedRemote(t, "v3.19.0", "CC-Switch-v3.19.0-Windows-Portable.zip", int64(len(zipBytes)), shaHex(zipBytes))
	src.set(zipBytes, false)
	if err := m.Download(newTxnID(), "v3.19.0", nil); err != nil {
		t.Fatalf("首装: %v", err)
	}
	if err := m.Download(newTxnID(), "v3.19.0", nil); err != nil {
	...
	seedRemote(t, "v3.19.0", "CC-Switch-v3.19.0-Windows-Portable.zip", int64(len(tampered)), shaHex(tampered))
	src.set(tampered, false)
	err := m.Download(newTxnID(), "v3.19.0", nil)
	if err == nil || !strings.Contains(err.Error(), "漂移") {
		t.Fatalf("同版本异摘要应拒绝覆盖, got %v", err)
	}
}

// TestZipLayoutRejectedKeepsTree 便携锚点不符的包（缺 portable.ini）：解包进
// staging 后自检拒装，最终目录不出现、半件不残留。
func TestZipLayoutRejectedKeepsTree(t *testing.T) {
	src := newZipSource(t)
	m := newChainManager(t, src)

	noMark := makeTestZip(t, map[string]string{exeName: "fake-exe"}) // 缺 portable.ini
	zipBytes, err := os.ReadFile(noMark)
	if err != nil {
		t.Fatal(err)
	}
	seedRemote(t, "v3.20.0", "CC-Switch-v3.20.0-Windows-Portable.zip", int64(len(zipBytes)), shaHex(zipBytes))
	...
	err = m.Download(newTxnID(), "v3.20.0", nil)
	if err == nil || !strings.Contains(err.Error(), "便携标记") {
		t.Fatalf("缺绿色版锚点应拒装, got %v", err)
	}
	if _, statErr := os.Stat(filepath.Join(m.versionsDir, "ccswitch_3.20.0")); !os.IsNotExist(statErr) {
		t.Error("锚点不符的包不得落位")
	}
