...
func (s *CCSwitchService) DownloadVersion(targetVersion string) (string, error) {
	...
	go func() {
		...
		if lerr != nil {
			if app := application.Get(); app != nil && app.Event != nil {
				app.Event.Emit("ccswitch:version-download", version.DownloadProgress{Version: targetVersion, Stage: "error", Message: lerr.Error()})
			}
			notify.Error("ccswitch", "版本下载失败", fmt.Sprintf("CC Switch %s 后台租约开启失败: %v", targetVersion, lerr), "/ext/ccswitch")
			return
		}
...
func (s *CCSwitchService) ImportLocal(srcDir string) (version.CCVersionInfo, error) {
	...
	if err != nil {
		return version.CCVersionInfo{}, err
	}
