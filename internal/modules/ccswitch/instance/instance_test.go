...
// 本文件聚焦 ccswitch 适配层：内核快照 → 本包 Snapshot 的形状/词表映射、
// ErrExternal 契约映射、"已手动停止"文案折回、信使接缝、WM_CLOSE 宽限收口，
// 以及维持既有环境口径的真进程冒烟。
...
func TestExitCodeAndMessageMapping(t *testing.T) {
	...
	e.onSupState(sup.Snapshot{State: sup.StateFailed, PID: 0, Version: "v3.20.0",
		Error: "托管进程异常退出（退出码 1）"})
	snap := rec.last()
	if snap.State != StateFailed || snap.ExitCode != 1 {
		t.Fatalf("failed 映射异常: %+v", snap)
	}
	if !strings.Contains(snap.Error, "CC Switch 异常退出（退出码 1）") || !strings.Contains(snap.Error, "WebView2") {
		t.Errorf("异常退出文案应保持 ccswitch 既有口径: %s", snap.Error)
	}
...
func TestStartEmptyExeRejected(t *testing.T) {
	...
	if err := e.Start(StartOptions{Version: "v3.20.0"}); err == nil ||
		err.Error() != "cc-switch.exe 路径不能为空" {
...
func TestStartCreateJobFail(t *testing.T) {
	...
	err := e.Start(StartOptions{Version: "v3.20.0", Exe: mustCmdExe(t)})
	if err == nil {
...
// 确定性命令拉起 cmd.exe（specArgs 缝注入）→ running → OpenWindow 信使
// （接缝打桩）→ Stop 强杀 → stopped。
func TestManagedLifecycleSmoke(t *testing.T) {
	...
	exe := mustCmdExe(t)
	if err := e.Start(StartOptions{Version: "v3.20.0", Exe: exe}); err != nil {
...
func TestQuitGraceFallback(t *testing.T) {
	...
	e.specArgs = smokeCmdArgs
	if err := e.Start(StartOptions{Version: "v3.20.0", Exe: mustCmdExe(t)}); err != nil {
		t.Fatalf("Start: %v", err)
	}
	defer func() { _ = e.Stop() }()
	waitState(t, e, StateRunning)
...
// 按 ccswitch 既有契约映射为成功无操作，状态校正为 external。
