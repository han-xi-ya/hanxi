...
const (
	...
	StateExternal State = "external" // 外部用户自启的 CC Switch 主实例（非本引擎托管）
)
...
// manualStopWording 内核手动停止的收口文案；cc-switch 既有快照口径在 stopped
...
func (o StartOptions) validate() error {
	if o.Exe == "" {
		return fmt.Errorf("cc-switch.exe 路径不能为空")
	}
	return nil
}
...
// Engine CC Switch 单实例运行引擎：组合内核 supervisor.Engine，
// 本层持有 CC Switch 专属账目（退出码/停止时刻）与信使唤窗、WM_CLOSE 优雅退出钩子。
type Engine struct {
	mu        sync.Mutex
	...
	// specArgs 真机冒烟专用注入缝：生产 CC Switch 恒无参拉起（nil，无参即开
...
func NewEngine(jobAPI platform.JobAPI, probe CCSwitchProbe, cb Callbacks) *Engine {
	...
	// 优雅退出通道：Quit 的 grace 窗口内先向单实例消息窗口投递 WM_CLOSE
	// （tauri CloseRequested 按用户托盘设置 exit(0) 或 hide），超时由内核强杀兜底。
...
// CC Switch 无后台启动 CLI，唯一启动语义即"无参拉起 → 主窗口显示"。
...
func (e *Engine) Start(opts StartOptions) error {
	...
	// 刻意不设置 HideWindow 等窗口干预：CC Switch 是 GUI 子系统程序，既不产生
...
// OpenWindow 完成单实例协议的窗口唤起：拉起同路径第二个 CC Switch 实例充当
...
// 兜底（驻留托盘场；CC Switch 数据为 SQLite WAL + config.json 原子写，进程级
// 终止由 SQLite crash-safe 保障）。幂等；external 态按既有契约映射为无操作成功。
...
// WaitReady 阻塞等待 CC Switch 主实例就绪（单实例互斥体出现），超时返回 false。
func (e *Engine) WaitReady(timeout time.Duration) bool {
	return e.probe.WaitForReady(timeout)
}

// IsMainWindowOpen 主窗口是否可见——空闲自动退出的豁免信号（探针领域能力，不经内核）。
...
//	supervisor stopping → running（ccswitch 既有词表无 stopping：终止窗口对前端保持
...
// mapErrorMessage 还原 ccswitch 既有失败文案：
//   - 内核异常退出消息改回"CC Switch 异常退出（退出码 N）。请确认已安装
//     WebView2 Runtime"；
...
func mapErrorMessage(s sup.Snapshot) string {
	if s.State == sup.StateFailed {
		if code, ok := exitCodeFromKernelMessage(s.Error); ok {
			return fmt.Sprintf("CC Switch 异常退出（退出码 %d）。请确认已安装 WebView2 Runtime", code)
		}
	}
