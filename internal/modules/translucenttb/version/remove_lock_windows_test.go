...
// TestRemoveRefusedWhileExeLocked 文件锁拒卸：版本目录内 TranslucentTB.exe 被非共享
...
func TestRemoveRefusedWhileExeLocked(t *testing.T) {
	...
	installViaChain(t, m, src, "2026.2", "locked-exe")

	exePath := filepath.Join(m.versionsDir, "translucenttb_2026.2", exeName)
	...
	if rmErr := m.Remove("2026.2"); rmErr == nil {
