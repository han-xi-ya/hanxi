// Wave 4-B 安装事务 journal 链路测试（与 markeron/ccswitch 同构、资产形态为便携 zip）：
...
// translucenttbTestSteps 与 TranslucentTBService.translucenttbInstallSteps 同一词表
// （服务层编排在本包不可达，此处复刻同款阶段映射以验证交接面；verify 由内核
// Fetch 折进 download 步，不造幻影步骤）。
var translucenttbTestSteps = []string{"download", "unpack", "place"}
...
func driveInstall(t *testing.T, m *Manager, txnID, version string) error {
	t.Helper()
	txn, err := ops.BeginTxn("translucenttb", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		version, txnID, translucenttbTestSteps)
	if err != nil {
		return err
	}
	stepIdx := -1
...
func TestJournalChainSuccess(t *testing.T) {
	...
	if err := driveInstall(t, m, txnID, "2026.2"); err != nil {
	...
	if j.State != operation.TxnSucceeded || j.Operation != operation.TxnOpInstall ||
		j.DeliveryKind != extapi.DeliveryManagedDeclarative || j.ModuleID != "translucenttb" {
		t.Errorf("账本终态异常: %+v", j)
	}
	...
	_, hub := ops.Kernel()
	rec := hub.Recent("translucenttb", 10)
...
func TestJournalChainFailureInterrupt(t *testing.T) {
	...
	src := newZipSource(t)
	m := newChainManager(t, src)
	installViaChain(t, m, src, "2026.1", "old-exe")
...
func TestJournalSingleWriteRejectsConcurrent(t *testing.T) {
	withTestKernel(t)
	if _, err := ops.BeginTxn("translucenttb", extapi.OpInstall, extapi.DeliveryManagedDeclarative, "2026.1", newTxnID(), translucenttbTestSteps); err != nil {
		t.Fatal(err)
	}
	_, err := ops.BeginTxn("translucenttb", extapi.OpUpdate, extapi.DeliveryManagedDeclarative, "2026.2", newTxnID(), translucenttbTestSteps)
...
func TestJournalCrashRecoveryEndorsement(t *testing.T) {
	...
	txnID := newTxnID()
	txn, err := ops.BeginTxn("translucenttb", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		"2026.2", txnID, translucenttbTestSteps)
	if err != nil {
		t.Fatal(err)
	}
	txn.Step(0)
	...
	trees := map[string]*artifact.Tree{"translucenttb": OpenTree(m.versionsDir)}
	...
	rec := hub2.Recent("translucenttb", 0)
