// Wave 4-B 安装事务 journal 链路测试（与 ccswitch 同构、资产形态为 MSI 管理
// 提取特例）：假下载源 + 假 msiExtract 驱动 Begin→Advance→Complete 全步、
// 中断失败收口、强杀模拟（重启 Store 后 Pending 可见）与启动恢复"背书"清理。
// 全部真 tmp 目录 + 回环假源，无网络依赖、不依赖本机 Installer 服务。
package version
...
// keyvizTestSteps 与 KeyvizService.keyvizInstallSteps 同一词表（服务层编排在
// 本包不可达，此处复刻同款阶段映射以验证交接面；verify（官方摘要双核）由内核
// Fetch 折进 download 步，不造幻影步骤；unpack 步对应 msiexec 管理提取段）。
var keyvizTestSteps = []string{"download", "unpack", "place"}
...
func driveInstall(t *testing.T, m *Manager, txnID, version string) error {
	t.Helper()
	txn, err := ops.BeginTxn("keyviz", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		version, txnID, keyvizTestSteps)
	if err != nil {
		return err
	}
	stepIdx := -1
...
func TestJournalChainSuccess(t *testing.T) {
	...
	if j.State != operation.TxnSucceeded || j.Operation != operation.TxnOpInstall ||
		j.DeliveryKind != extapi.DeliveryManagedDeclarative || j.ModuleID != "keyviz" {
		t.Errorf("账本终态异常: %+v", j)
	}
...
func TestJournalSingleWriteRejectsConcurrent(t *testing.T) {
	withTestKernel(t)
	if _, err := ops.BeginTxn("keyviz", extapi.OpInstall, extapi.DeliveryManagedDeclarative, "v2.1.0", newTxnID(), keyvizTestSteps); err != nil {
		t.Fatal(err)
	}
	_, err := ops.BeginTxn("keyviz", extapi.OpUpdate, extapi.DeliveryManagedDeclarative, "v2.1.1", newTxnID(), keyvizTestSteps)
...
func TestJournalCrashRecoveryEndorsement(t *testing.T) {
	...
	trees := map[string]*artifact.Tree{"keyviz": OpenTree(m.versionsDir)}
