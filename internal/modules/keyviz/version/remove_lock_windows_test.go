...
// TestRemoveRefusedWhileExeLocked 文件锁拒卸：版本目录内 keyviz.exe 被非共享
...
func TestRemoveRefusedWhileExeLocked(t *testing.T) {
	...
	installViaChain(t, m, src, "v2.1.1", "locked-exe")

	exePath := filepath.Join(m.versionsDir, "keyviz_2.1.1", exeName)
	...
	if rmErr := m.Remove("v2.1.1"); rmErr == nil {
