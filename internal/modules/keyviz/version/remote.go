...
// keyviz 的 v2.0.0a / v2.0.0a2 / v2.0.0a3 预发布系列天然被本规则挡在门外。
...
// findMSIAsset 从 release 资产中筛选 Windows MSI 安装包。
// 上游不发便携 zip（唯一 zip 停留在 v2.0.0a3 预发布），Windows 正式版恒为
// keyviz_<ver>_windows.msi——发布 workflow 定制命名，无 tauri 默认的 _x64_en-US
// 后缀。精确形状天然排除 _macos.dmg / -linux.deb / -linux.rpm 等平台资产。
func findMSIAsset(assets []asset, version string) (asset, bool) {
	ver := strings.TrimPrefix(version, "v")
	needName := strings.ToLower("keyviz_" + ver + "_windows.msi")
	...
	for _, a := range assets {
		...
		if strings.HasSuffix(lower, "_windows.msi") && strings.Contains(lower, "_"+ver+"_") {
			return a, true
		}
	}
	return asset{}, false
...
func (c *releaseCache) get() ([]KeyvizRelease, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// 过滤规则：draft / 非纯语义 tag / 无 Windows MSI 资产 / 官方 sha256 缺失的 release 一律不入列表。
func parseReleasesBody(body []byte) ([]KeyvizRelease, error) {
	var releases []release
	if err := json.Unmarshal(body, &releases); err != nil {
		return nil, fmt.Errorf("parse releases: %w", err)
	}

	var list []KeyvizRelease
	for _, r := range releases {
		...
		// （GitHub 自 2024 年起所有新资产均带 digest，本仓库实测 v2.0.0a 起全覆盖）
		sha := strings.TrimPrefix(strings.ToLower(arch.Digest), digestPrefix)
		if len(sha) != 64 {
			continue
		}
		list = append(list, KeyvizRelease{
			Version:   r.TagName,
