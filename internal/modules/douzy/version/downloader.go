...
// assetMirrors 构造直连与镜像下载 URL 候选列表（与 rustdesk/ccswitch 同一组镜像前缀）。
// version 参数须传上游原始 tag（desktop-vX.Y.Z），assetName 为资产文件名。
...
// 无断点续传：每轮每源均从头重下（Setup 单文件 ~147MB，中途断连即该源本轮作废）。
...
func tryDownloadSingle(client *http.Client, url, dest string, onProgress func(done int64)) error {
	...
	case resp.StatusCode >= 300 && resp.StatusCode < 400:
		// /releases/download/ 正常由 client 跟随 302 到 CDN 取 200；
		// 直接拿到 3xx 说明该镜像不代理此资产，立即换下一源。
...
// fileSHA256 计算文件 sha256（记录进 meta 供事后诊断；下载校验使用官方 release digest）。
...
// verifyPEMagic 校验文件头 MZ 魔数：安装包 exe 无 zip 布局可自检，
