...
const (
	exeName   = "ddns-go.exe"
	dirPrefix = "ddnsgo_" // 版本隔离目录前缀（与 ccswitch_ / frp_ / markeron_ 同构）
)
...
// dirNameRe 版本目录名（ddnsgo_6.17.6）；imported- 分支收纳时间戳兜底的导入
var dirNameRe = regexp.MustCompile(`^` + dirPrefix + `(?:[0-9][0-9a-zA-Z.]+|imported-\d{8}-\d{6})$`)

// Manager ddns-go 版本管理引擎：远程列表、下载完整性校验、保布局解压隔离、本地导入。
type Manager struct {
	versionsDir string
