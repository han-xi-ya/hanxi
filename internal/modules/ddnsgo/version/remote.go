...
// plainSemverTag 严格纯语义版本 tag（格式 vX.Y.Z），防 beta/nightly 等非规范 tag 污染版本列表
...
// portableSuffix ddns-go 资产命名形如 ddns-go_6.17.6_windows_x86_64.zip
// （tag 带 v 前缀、资产名不带）。后缀天然排除 linux/darwin 的 .tar.gz、
// windows 的 arm64/i386 变体与 .msi（上游不发 msi，防御性兜底）。
const portableSuffix = "_windows_x86_64.zip"

// findPortableAsset 从 release 资产中筛选 Windows x64 便携 zip。
func findPortableAsset(assets []asset, version string) (asset, bool) {
	ver := strings.TrimPrefix(version, "v")
	needName := strings.ToLower("ddns-go_" + ver + portableSuffix)
	...
	for _, a := range assets {
		...
		if strings.HasSuffix(lower, portableSuffix) && strings.Contains(lower, strings.ToLower(ver)) {
			return a, true
		}
	}
	return asset{}, false
...
func (c *releaseCache) get() ([]DdnsRelease, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// 过滤规则：draft / 非纯语义 tag / 无 x64 zip 资产 / 官方 sha256 缺失的 release 一律不入列表。
func parseReleasesBody(body []byte) ([]DdnsRelease, error) {
	var releases []release
	if err := json.Unmarshal(body, &releases); err != nil {
		return nil, fmt.Errorf("parse releases: %w", err)
	}

	var list []DdnsRelease
	for _, r := range releases {
		...
		// （实测 v6.16.11~v6.17.6 连续 10 个版本全资产带 digest）
		sha := strings.TrimPrefix(strings.ToLower(arch.Digest), digestPrefix)
		if len(sha) != 64 {
			continue
		}
		list = append(list, DdnsRelease{
			Version:   r.TagName,
