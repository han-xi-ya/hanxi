...
// 冷启动自动回退最新已装）与 followOnExit。与 picliteStore 同款原子写
// （tmp+rename），损坏容忍（解析失败按空配置继续，不阻断模块）。
...
func (s *guoheviewStore) load() error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *guoheviewStore) saveLocked() error {
	dir := filepath.Dir(s.filePath)
	if err := os.MkdirAll(dir, 0755); err != nil {
		return err
	}
	bytes, err := json.MarshalIndent(guoheviewConfig{ActiveVersion: s.activeVersion, FollowOnExit: &s.followOnExit}, "", "  ")
	if err != nil {
		return err
	}
...
func (s *guoheviewStore) GetActive() string {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *guoheviewStore) SetActive(version string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *guoheviewStore) SetFollowOnExit(b bool) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.followOnExit = b
	return s.saveLocked()
