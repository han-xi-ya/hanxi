...
// 会话窗口）。取消边界如实标注：WindTerm 开着活动会话时关窗会弹断开确认
// 框，WM_CLOSE 被模态框挡住不再退出——调用方以宽限期收口：自有实例走
// supervisor 强杀兜底（会话由远端超时回收），外部实例走 externalquit
// confirm-force 档在用户明确知情后执行。窗口不存在静默返回 0。
