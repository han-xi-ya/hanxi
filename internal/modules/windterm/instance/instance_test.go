...
// 说明：进程治理主流程由内核 supervisor 测试矩阵覆盖。本文件聚焦 windterm
...
func TestMapKernelStateVocabulary(t *testing.T) {
	...
	// 窗口外异常退出：WindTerm 文案（引导回版本管理重装）
	e.onSupState(sup.Snapshot{State: sup.StateFailed, Error: "托管进程异常退出（退出码 2）"})
	if got := rec.last(); got.State != StateFailed || !strings.Contains(got.Error, "WindTerm 异常退出（退出码 2）") {
...
func TestStoppedMessageAndPidShape(t *testing.T) {
	...
	e.onSupState(sup.Snapshot{State: sup.StateRunning, Version: "v2.7.0", PID: 4242, Exe: `C:\v\WindTerm.exe`, Since: time.Now()})
	run := rec.last()
	if run.PID != 4242 || run.ExePath != `C:\v\WindTerm.exe` || run.Version != "v2.7.0" {
		t.Fatalf("running 快照形状异常: %+v", run)
	}
	e.onSupState(sup.Snapshot{State: sup.StateStopped, Version: "v2.7.0", PID: 4242, Exe: `C:\v\WindTerm.exe`, Error: "已手动停止"})
	stop := rec.last()
...
func TestFinishQuitAttributionTable(t *testing.T) {
	...
	snap := sup.Snapshot{State: sup.StateRunning, PID: 42, Exe: `C:\v\WindTerm.exe`}
...
func TestVerifyTokenRejectsPathMismatch(t *testing.T) {
	...
	err := e.verifyToken(platform.VerifyToken{PID: 10, ExePath: filepath.Join(dir, "WindTerm.exe"), StartedAt: proc.info.StartedAt})
...
func TestQuitExternalDeclinedKeepsExternal(t *testing.T) {
	...
	info := platform.ProcInfo{PID: 9001, ExePath: `C:\ext\WindTerm.exe`, StartedAt: time.Now()}
...
func TestQuitExternalForcedAfterConsent(t *testing.T) {
	...
	info := platform.ProcInfo{PID: 9002, ExePath: `C:\ext\WindTerm.exe`, StartedAt: time.Now()}
	proc := &fakeProcessAPI{info: info}
...
func TestQuitExternalBlockedIsHonest(t *testing.T) {
	...
	info := platform.ProcInfo{PID: 9003, ExePath: `C:\ext\WindTerm.exe`, StartedAt: time.Now()}
...
func TestStartRejectsEmptyExe(t *testing.T) {
	e := NewEngine(&fakeJobAPI{}, &fakeProcessAPI{}, &fakeProbe{}, Callbacks{})
	if err := e.Start(StartOptions{Version: "v2.7.0"}); err == nil || err.Error() != "WindTerm.exe 路径不能为空" {
...
func TestStartRejectsWhenActive(t *testing.T) {
	...
	if err := e.Start(StartOptions{Version: "v2.7.0", Exe: cmdExe, Detached: true}); err == nil ||
		err.Error() != "本会话启动的 WindTerm 已在运行" {
