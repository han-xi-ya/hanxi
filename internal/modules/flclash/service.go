...
func (s *FlClashService) DownloadVersion(targetVersion string) (string, error) {
	...
	go func() {
		...
		if lerr != nil {
			if app := application.Get(); app != nil && app.Event != nil {
				app.Event.Emit("flclash:version-download", version.DownloadProgress{Version: targetVersion, Stage: "error", Message: lerr.Error()})
			}
			notify.Error("flclash", "版本下载失败", fmt.Sprintf("FlClash %s 后台租约开启失败: %v", targetVersion, lerr), "/ext/flclash")
			return
		}
		...
		if err := txn.Done(); err != nil {
			emit(version.DownloadProgress{Version: targetVersion, Stage: "error", Message: err.Error()})
			notify.Error("flclash", "版本下载失败", fmt.Sprintf("FlClash %s 事务收口失败: %v", targetVersion, err), "/ext/flclash")
			return
		}
...
func (s *FlClashService) ImportLocal(srcDir string) (version.FlClashVersionInfo, error) {
	...
	if err != nil {
		return version.FlClashVersionInfo{}, err
	}
