...
// flclashTestSteps 与 FlClashService.flclashInstallSteps 同一词表（服务层
// 编排在本包不可达，此处复刻同款阶段映射以验证交接面；verify 由内核 Fetch
// 折进 download 步，不造幻影步骤）。
var flclashTestSteps = []string{"download", "unpack", "place"}
...
func driveInstall(t *testing.T, m *Manager, txnID, version string) error {
	t.Helper()
	txn, err := ops.BeginTxn("flclash", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		version, txnID, flclashTestSteps)
	if err != nil {
		return err
	}
	stepIdx := -1
...
func TestJournalChainSuccess(t *testing.T) {
	...
	if j.State != operation.TxnSucceeded || j.Operation != operation.TxnOpInstall ||
		j.DeliveryKind != extapi.DeliveryManagedDeclarative || j.ModuleID != "flclash" {
		t.Errorf("账本终态异常: %+v", j)
	}
...
func TestJournalChainFailureInterrupt(t *testing.T) {
	...
	src := newZipSource(t)
	m := newChainManager(t, src)
	installViaChain(t, m, src, "0.8.95", "old-exe")

	txnID := newTxnID()
	newZip := buildFlClashZip(t, "interrupted")
...
func TestJournalSingleWriteRejectsConcurrent(t *testing.T) {
	withTestKernel(t)
	if _, err := ops.BeginTxn("flclash", extapi.OpInstall, extapi.DeliveryManagedDeclarative, "0.8.96", newTxnID(), flclashTestSteps); err != nil {
		t.Fatal(err)
	}
	_, err := ops.BeginTxn("flclash", extapi.OpUpdate, extapi.DeliveryManagedDeclarative, "0.8.97", newTxnID(), flclashTestSteps)
...
func TestJournalCrashRecoveryEndorsement(t *testing.T) {
	...
	trees := map[string]*artifact.Tree{"flclash": OpenTree(m.versionsDir)}
