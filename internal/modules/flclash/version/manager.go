...
const (
	exeName   = "FlClash.exe"
	dirPrefix = "flclash_" // 版本隔离目录前缀（与 bcu_6.2.0 / ccswitch_3.20.0 同构）
)

// dirNameRe 版本目录名（flclash_0.8.96）；
...
// Manager FlClash 版本管理引擎：远程列表、下载完整性校验、保布局解压隔离、本地导入。
type Manager struct {
	versionsDir string
	client      *http.Client // 下载客户端（长超时；便携包约 58MB）
}
...
func (m *Manager) Download(version string, onProgress func(p DownloadProgress)) error {
	...
	// 2. 下载 zip（直连 + 镜像逐个回退；tag 与资产版本同形，用 release 自带 tag 拼路径）
...
func (m *Manager) ImportLocal(srcDir string) (FlClashVersionInfo, error) {
	...
	// 黑名单整搬：临时/系统垃圾文件不搬，其余（exe/dll/data/等）全量保留
	var copied int
