...
// TestRemoveRefusedWhileExeLocked 文件锁拒卸：版本目录内 FlClash.exe 被非共享
...
func TestRemoveRefusedWhileExeLocked(t *testing.T) {
	...
	installViaChain(t, m, src, "0.8.95", "locked-exe")

	exePath := filepath.Join(m.versionsDir, "flclash_0.8.95", exeName)
	...
	if rmErr := m.Remove("0.8.95"); rmErr == nil {
	...
	if err := m.Remove("0.8.95"); err != nil {
