...
type StartOptions struct {
	...
	Detached bool
	Exe      string // 主程序绝对路径（便携=版本隔离目录内 packer；安装版=Program Files 下 SubnetDesk.exe）
}

func (o StartOptions) validate() error {
	if o.Exe == "" {
		return fmt.Errorf("subnetdesk.exe 路径不能为空")
	}
	return nil
}
...
type Engine struct {
	...
	probe     SubnetDeskProbe
	cb        Callbacks
}

func NewEngine(jobAPI platform.JobAPI, probe SubnetDeskProbe, cb Callbacks) *Engine {
	return &Engine{
		state:  StateStopped,
		jobAPI: jobAPI,
		probe:  probe,
	...
	for {
		...
		if since >= startGrace || (exitCode != 0 && since >= earlyFailWait) {
			e.finish(StateFailed, fmt.Sprintf(
				"SubnetDesk 进程树未能出现（外层退出码 %d）。首次启动需解压内置负载，"+
