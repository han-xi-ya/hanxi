...
// vscodeTestSteps 与 VSCodeService.vscodePortableInstallSteps 同一词表（服务层
// 编排在本包不可达，此处复刻同款阶段映射以验证交接面；verify 由内核 Fetch
// 折进 download 步，不造幻影步骤）。
var vscodeTestSteps = []string{"download", "unpack", "place"}
...
// driveInstall 复刻 service.DownloadVersion 后台协程的事务编排（便携形态）。
func driveInstall(t *testing.T, m *Manager, txnID, version string) error {
	t.Helper()
	txn, err := ops.BeginTxn("vscode", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		version, txnID, vscodeTestSteps)
	if err != nil {
		return err
	}
	stepIdx := -1
	...
	if err := m.Download(txnID, version, FormPortable, emit); err != nil {
...
func TestJournalChainSuccess(t *testing.T) {
	...
	txnID := newTxnID()
	zipBytes := buildVSZip(t, "journal-exe")
	seedAndStage(t, m, src, "1.136.1", zipBytes, shaHex(zipBytes))

	if err := driveInstall(t, m, txnID, "1.136.1"); err != nil {
	...
	if j.State != operation.TxnSucceeded || j.Operation != operation.TxnOpInstall ||
		j.DeliveryKind != extapi.DeliveryManagedDeclarative || j.ModuleID != "vscode" {
		t.Errorf("账本终态异常: %+v", j)
	}
...
func TestJournalChainFailureInterrupt(t *testing.T) {
	...
	src := newZipSource(t)
	m := NewManager(t.TempDir())
	installViaChain(t, m, src, "1.135.0", "old-exe")

	txnID := newTxnID()
	newZip := buildVSZip(t, "interrupted")
	seedAndStage(t, m, src, "1.136.1", newZip, shaHex(newZip))
...
func TestJournalSingleWriteRejectsConcurrent(t *testing.T) {
	withTestKernel(t)
	if _, err := ops.BeginTxn("vscode", extapi.OpInstall, extapi.DeliveryManagedDeclarative, "1.135.0", newTxnID(), vscodeTestSteps); err != nil {
		t.Fatal(err)
	}
	_, err := ops.BeginTxn("vscode", extapi.OpUpdate, extapi.DeliveryManagedDeclarative, "1.136.1", newTxnID(), vscodeTestSteps)
...
func TestJournalCrashRecoveryEndorsement(t *testing.T) {
	...
	trees := map[string]*artifact.Tree{"vscode": OpenTree(m.versionsDir)}
