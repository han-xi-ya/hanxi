...
func (s *EverythingService) DownloadVersion(targetVersion string) (string, error) {
	...
	go func() {
		...
		if lerr != nil {
			s.emitDownload(DownloadTicket{Component: "app", Version: targetVersion, Stage: "error", Message: lerr.Error()})
			notify.Error("everything", "版本下载失败", fmt.Sprintf("Everything %s 后台租约开启失败: %v", targetVersion, lerr), "/ext/everything")
			return
		}
		...
		if err := drive(txn.Context(), txnID, targetVersion, emit); err != nil {
			...
			if txn.Err() != nil {
				txn.Fail("operation-cancelled", "托管操作已取消")
				emit(evversion.DownloadProgress{Version: targetVersion, Stage: "error", Message: "托管下载已取消"})
			} else {
