// Wave 4-B 安装事务 journal 链路测试（与 ccswitch 同构、资产形态为 electron
// unpacked zip）：假下载源驱动 Begin→Advance→Complete 全步、中断失败收口、
// 强杀模拟（重启 Store 后 Pending 可见）与启动恢复"背书"清理。全部真 tmp
// 目录 + 回环假源，无网络依赖。
package version
...
// paseoTestSteps 与 PaseoService.paseoInstallSteps 同一词表（服务层编排在本包
// 不可达，此处复刻同款阶段映射以验证交接面；verify 对应内核 Fetch 官方摘要
// 双核的真实阶段迁移）。
var paseoTestSteps = []string{"download", "verify", "unpack", "place"}
...
func driveInstall(t *testing.T, m *Manager, txnID, version string) error {
	t.Helper()
	txn, err := ops.BeginTxn("paseo", extapi.OpInstall, extapi.DeliveryManagedDeclarative,
		version, txnID, paseoTestSteps)
	if err != nil {
		return err
	}
	stepIdx := -1
...
// TestJournalChainSuccess 正例全步链：四步推进、succeeded 收口；观察面记录同步。
func TestJournalChainSuccess(t *testing.T) {
	...
	if j.State != operation.TxnSucceeded || j.Operation != operation.TxnOpInstall ||
		j.DeliveryKind != extapi.DeliveryManagedDeclarative || j.ModuleID != "paseo" {
		t.Errorf("账本终态异常: %+v", j)
	}
...
func TestJournalChainFailureInterrupt(t *testing.T) {
	...
	src := newZipSource(t)
	m := newChainManager(t, src)
	installViaChain(t, m, src, "0.7.2", "v0.7.2", "old-exe")

	txnID := newTxnID()
	newZip := buildElectronZip(t, "interrupted")
...
func TestJournalSingleWriteRejectsConcurrent(t *testing.T) {
	withTestKernel(t)
	if _, err := ops.BeginTxn("paseo", extapi.OpInstall, extapi.DeliveryManagedDeclarative, "0.7.2", newTxnID(), paseoTestSteps); err != nil {
		t.Fatal(err)
	}
	_, err := ops.BeginTxn("paseo", extapi.OpUpdate, extapi.DeliveryManagedDeclarative, "0.8.0", newTxnID(), paseoTestSteps)
...
func TestJournalCrashRecoveryEndorsement(t *testing.T) {
	...
	trees := map[string]*artifact.Tree{"paseo": OpenTree(m.versionsDir)}
