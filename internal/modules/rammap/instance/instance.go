// Package instance 实现 RAMMap 自有实例的进程生命周期托管。
//
// 进程治理主流程（spawn → Job Object 绑定 → 退出分类 → 强制终止兜底）收口至
// 共享内核 hanxi/packages/go/supervisor；本包只保留 RAMMap 领域适配：
//   - 多实例观察工具（无单实例承诺，二次拉起各开一窗）：引擎只追踪自己拉起
//     的那一个进程，用户自行启动的实例归 external 感知；外部归属判定按探针
//     PID 排除自有（supProbe），退出瞬间 Toolhelp 快照仍列出自己的竞态不会
//     误判"外部接管"；
//   - 提权三重契约（#17，载荷 manifest=requireAdministrator）：未提权 Hanxi
//     spawn 直拒 740 且系统不代弹 UAC——spawn 失败广播暂扣改写为含
//     "管理员"关键词的指引文案（litemonitor 同族手法：ElevateRestart 一键
//     提权重启组件据此关键词挂载）；提权态 Hanxi 拉起后同完整性等级，
//     Job 绑定/WM_CLOSE/Terminate 全部正常可用；
//   - external 实例退出走 QuitExternal（委托 packages/go/externalquit 按
//     N3 终裁分档执行）：RAMMap 是零状态观察工具（打开即重扫内存统计，
//     无保存概念），按 force-free 档治理——优雅 WM_CLOSE 不生效即直接强杀，
//     不打扰用户；提权目标（外部以管理员自启的实例）UIPI 拦截如实降级；
//   - 分层退出归因（QuitResult）：投递/强杀前身份复核、WM_CLOSE、宽限窗口
//     均为模块策略。关窗即退（无托盘驻留、无确认框，Sysinternals 工具族
//     行为），宽限只防消息循环瞬时卡顿；
//   - "打开窗口"无信使语义（多实例无转发）：external 态按 PID 枚举唤回已有
//     窗口，唤不回如实回指引。
//
// 本包零框架依赖，便于单元测试。
package instance
...
// closeGracePeriod Quit 的 WM_CLOSE 优雅退出宽限：RAMMap 关窗即退
// （无确认框、无落盘义务），宽限只防消息循环瞬时卡顿；包级变量供单测
// 压缩，生产 2s。
...
type StartOptions struct {
	...
	Exe      string // 载荷 exe 绝对路径（RAMMap64.exe / RAMMap64a.exe，托管目录内）
	Detached bool   // 独立运行：解除 JobObject 退出联动（"不随 Hanxi 关闭"开关）
}

func (o StartOptions) validate() error {
	if o.Exe == "" {
		return fmt.Errorf("%s 路径不能为空", payloadImageName())
	}
	return nil
}
...
// Engine RAMMap 自有实例运行引擎：组合内核 supervisor.Engine，本层持有
// RAMMap 专属账目（退出归因标记、宽限收口通道、异常退出码/停止时刻、
...
type Engine struct {
	...
	// spawn 失败广播暂扣账目（740 提权特判的收口窗口，litemonitor 同族手法）：
	// 内核 spawn 失败原始文案不含提权指引，elevateHint 需对 Start 返回错误链
	// 做 errors.As 判级（errno 文本随系统语言变化不可靠字符串匹配）——Start
	// 在途时命中 spawn 失败前缀的 failed 广播暂存此处，待 Start 返回后改写补播。
	holdSpawnFail   bool
...
func (e *Engine) Start(opts StartOptions) error {
	...
	if st := e.sup.Snapshot().State; st == sup.StateRunning || st == sup.StateStarting || st == sup.StateStopping {
		return fmt.Errorf("本会话启动的 RAMMap 已在运行")
	}
	e.mu.Lock()
	...
	spawnErr := e.sup.Start(context.Background(), sup.Spec{
	...
	e.mu.Unlock()
	if heldOK && spawnErr != nil {
		// requireAdministrator + 未提权父进程：740 直译为"要求提升"不友好，
		// 改写为指向"以管理员身份重新启动 Hanxi"的指引文案后补播（词表零漂移，
		// "管理员"关键词同时喂养前端 ElevateRestart 一键通道）。
		if hint := elevateHint(spawnErr); hint != "" {
			held.Error = hint
		}
		e.mu.Lock()
...
// FocusExternal 唤回任一外部 RAMMap 可见窗口。只切前台，不接管、不绑 Job。
...
// failed 广播）。措辞耦合自 supervisor.Start 的 transition 前缀——内核文案变更
// 时本判定退化为不暂扣（原始透传广播照常送达），仅丢失 740 改写层，不崩溃。
...
// "RAMMap 异常退出（退出码 N）。若反复出现，请在版本管理重新安装"；
// 其余透传内核措辞。
func mapKernelError(msg string) string {
	...
	if code, ok := exitCodeFromKernelMessage(msg); ok {
		return fmt.Sprintf("RAMMap 异常退出（退出码 %d）。若反复出现，请在版本管理重新安装", code)
	}
	return msg
}
...
// supProbe 把 RAMMap Probe 适配为内核统一探针契约：回答"是否存在非自有的
// RAMMap 进程"并携带首个外部实例身份（external 快照展示与 QuitExternal
