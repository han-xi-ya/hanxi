...
// 本文件聚焦 recordly 适配层：内核快照 → 本包 Snapshot 的形状/词表映射、
// ErrExternal 契约映射、"已手动停止"文案折回、异常退出话术逐字还原、
// 信使拉起接缝、externalSettle 受管在途静默、RECORDLY_DISABLE_AUTO_UPDATES
// 的 Spec.Env 注入通道，以及维持既有环境口径的真进程冒烟。
...
func TestExitCodeAndMessageMapping(t *testing.T) {
	...
	e.onSupState(sup.Snapshot{State: sup.StateFailed, PID: 0, Version: "v1.3.5",
		Error: "托管进程异常退出（退出码 1）"})
	snap := rec.last()
	if snap.State != StateFailed || snap.ExitCode != 1 {
		t.Fatalf("failed 映射异常: %+v", snap)
	}
	// 铁律：异常退出话术逐字还原（未签名安装器 + 杀软拦截成因必须都在文案里）
	const wantPrefix = "Recordly 异常退出（退出码 1）。其安装器未数字签名，" +
		"若刚拉起即退出请检查杀毒软件是否拦截了未签名程序"
	if snap.Error != wantPrefix {
		t.Errorf("异常退出文案应保持 recordly 既有口径逐字一致:\n got %q\nwant %q", snap.Error, wantPrefix)
	}
...
func TestStartEmptyExeRejected(t *testing.T) {
	...
	if err := e.Start(StartOptions{Version: "v1.3.5"}); err == nil ||
		err.Error() != "Recordly.exe 路径不能为空" {
...
func TestStartCreateJobFail(t *testing.T) {
	...
	err := e.Start(StartOptions{Version: "v1.3.5", Exe: mustCmdExe(t)})
	if err == nil {
...
func TestManagedLifecycleSmoke(t *testing.T) {
	...
	if err := e.Start(StartOptions{Version: "v1.3.5", Exe: exe}); err != nil {
...
// TestEnvInjectionSmoke 托管核心契约冒烟：StartOptions.Env 经内核 Spec.Env
// 到达子进程环境——确定性命令探针：环境变量缺位即 exit 5（→ failed 入账），
// 在场则 exit 0（→ stopped）。断言落 stopped 即证明 RECORDLY_DISABLE_AUTO_UPDATES=1
// 注入通道全链可用（父环境经 t.Setenv 显式清空，消除宿主环境污染）。
func TestEnvInjectionSmoke(t *testing.T) {
	compressSettle(t)
	t.Setenv("RECORDLY_DISABLE_AUTO_UPDATES", "") // Windows 下空值=清除，父进程不携带

	e := NewEngine(newWindowsJobAPI(), &fakeProbe{}, Callbacks{})
	e.specArgs = []string{"/c", "if not defined RECORDLY_DISABLE_AUTO_UPDATES (exit 5) else (exit 0)"}
	if err := e.Start(StartOptions{
		Version: "v1.3.5",
		Exe:     mustCmdExe(t),
		Env:     []string{"RECORDLY_DISABLE_AUTO_UPDATES=1"},
	}); err != nil {
		t.Fatalf("Start: %v", err)
	}
	defer func() { _ = e.Stop() }()
	waitState(t, e, StateStopped)
	if snap := e.Snapshot(); snap.State != StateStopped {
		t.Fatalf("env 注入探针应以 exit 0 收口 stopped, got %+v", snap)
	}
}
...
func TestWaitAbnormalExitSmoke(t *testing.T) {
	...
	// 借 Args 让 cmd.exe 立即以退出码 1 收场（生产 Start 无参拉起 Recordly.exe）
	if err := e.sup.Start(context.Background(), sup.Spec{Version: "v1.3.5", Exe: mustCmdExe(t),
	...
	// 契约：失败文案必须指向未签名安装器/杀软拦截成因（真机首撞者靠它自查）
	if !strings.Contains(snap.Error, "Recordly 异常退出（退出码 1）") ||
		!strings.Contains(snap.Error, "杀毒软件") || !strings.Contains(snap.Error, "未数字签名") {
		t.Errorf("异常退出文案应保持 recordly 既有口径，实际: %q", snap.Error)
	}
}

// TestWaitExternalTaken 冷启动竞速冒烟：我方进程信使化自退（exit 0）后进程树
// 仍存活 → external（Recordly 二次拉起恰好就是这个语义分支）。
...
func TestInspectSettlesOnlyWhenManaged(t *testing.T) {
	...
	e.specArgs = []string{"/c", `for /l %i in (1,1,1500000) do @set x=%i`}
	if err := e.Start(StartOptions{Version: "v1.3.5", Exe: mustCmdExe(t)}); err != nil {
		t.Fatalf("Start: %v", err)
	}
	defer func() { _ = e.Stop() }()
	waitState(t, e, StateRunning)
...
// Recordly 窗口，postClose 静默 no-op）→ 宽限内未自然退（specArgs 纯 CPU 循环
// 保活，确定性走强杀兜底分支）→ JobObject 强杀收口，归 stopped 且不带话术。
...
// 按 recordly 既有契约映射为成功无操作，状态校正为 external。
...
// TestOpenWindowMessengerFailurePropagates 信使拉起失败 → OpenWindow 透传
// 既有包装文案（service 层再包"唤起窗口失败"）。
func TestOpenWindowMessengerFailurePropagates(t *testing.T) {
	...
	if opened, err := e.OpenWindow("whatever.exe"); err == nil || opened ||
		!strings.Contains(err.Error(), "拉起窗口信使失败") {
		t.Fatalf("信使失败应透传: %v %v", opened, err)
	}
}
...
func TestProbePassthrough(t *testing.T) {
	...
	if !e.IsMainWindowOpen() {
		t.Fatal("fake 探针 windowOpen=true 应透传为有窗")
	}
	...
	if e2.IsMainWindowOpen() {
		t.Fatal("fake 探针 windowOpen=false 应透传为无窗")
	}
}
