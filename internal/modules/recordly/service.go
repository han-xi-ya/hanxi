...
func (s *RecordlyService) DownloadVersion(targetVersion string) (string, error) {
	...
	go func() {
		...
		if lerr != nil {
			if app := application.Get(); app != nil && app.Event != nil {
				app.Event.Emit("recordly:version-download", version.DownloadProgress{Version: targetVersion, Stage: "error", Message: lerr.Error()})
			}
			notify.Error("recordly", "安装失败", fmt.Sprintf("Recordly %s 后台租约开启失败: %v", targetVersion, lerr), "/ext/recordly")
			return
		}
		...
		if err := txn.Done(); err != nil {
			emit(version.DownloadProgress{Version: targetVersion, Stage: "error", Message: err.Error()})
			notify.Error("recordly", "安装失败", fmt.Sprintf("Recordly %s 事务收口失败: %v", targetVersion, err), "/ext/recordly")
			return
		}
	}()
