...
- [x] 数据根策略：同级默认 + 绑定（F6）——默认落 exe 同级 `hanxidata/`（无则自动创建），可经 exe 同级 `hanxi.bind` 显式绑定其他位置；不可用时 fail loud 引导绑定，`%APPDATA%` 兜底与旧 `data/` 兼容均已退役（定稿见 `docs/plans/PLAN_PATHS.md`）
- [x] `extapi` 按需懒加载架构（未启用模块 0 内存与 0 协程占用，41 模块统一注册：40 常驻 + memo 条件项，2026-09-18 九件套后按 app.go 实数核）
...
> **九件套实际进度（2026-09-17 wave 三，详账见 `docs/PROGRESS.md`）**：已入库五件——F1 统一历史 / F2 剪贴板惯例 / F3 数据自动快照（含 memo 文件库化）/ F4 MCP 接入（F4a 无头 server + F4b 安装向导 + 「AI 接入」设置分区；剩 C9 三客户端真机红队闸门）/ F8 桌面留言板；随批修 S0-S2 与 R1 已清、R3（go.mod 离线绕行 replace 复核撤除）已随 #62 补记收口、R4（PLAN_MCP 落地回写）随 C10 交付，R2（安装前自检接线）进行中。开发中四件：F5 软件版本检测 / F6 数据目录同级化 / F7 hanxi-ocr 托管化 / F9 WSL USB 共享。
