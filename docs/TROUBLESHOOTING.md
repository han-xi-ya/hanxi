...
### 67. 内部调度函数导出即进绑定面：一个 time.Duration 参数让 wails3 生成出 dev 不存在的 `frontend/bindings/time/`（F9 USB 直通）
...
- **问题现象与错误原因**：`python - <<'EOF'` 看似走完，实际 `python` 指向 WindowsApps 商店存根，非交互直接 exit 49 且 stdout/stderr 全空；未用 `&&` 串链时后续 echo 仍打印，制造成功假象。
- **排查过程**：本应生成的文件不存在；`python -c "print(1)"` 同样 49 零输出坐实存根。
- **正确做法与标准修复方案**：改文件优先 Edit/Write；必须脚本处理时用 `py -3` 或真解释器绝对路径并检查退出码与产物。不可见字符 fixture 用纯 ASCII 字节切片表达。
- **避坑防重犯建议**：先跑解释器冒烟；shell 产物必须用 `ls/git status` 验尸，不能相信 echo。

### 70. 自动快照的“提交后再扫 mtime”会吞掉提交窗口内的新写入

- **问题现象与错误原因**：检查点开始后文件再次被写，当前版本实际只收进旧内容；若提交成功后才重扫最大 mtime 并推进 `scannedMT`，这次新写也被错误标成“已处理”，后续 tick 永久判 quiet。根因是把“本轮开始前已观测边界”和“提交结束时盘面”混成一个水位。
- **排查过程**：用 fake engine 在 `commit` hook 内二次写白名单文件，旧实现提交后 `ScanMtime` 恰好读到第二次写的 mtime，下一 tick 不再拍。
- **正确做法与标准修复方案**：执行前采集本轮候选 mtime；成功/无内容变化后最多推进到该候选，提交期间更大的 mtime 留给下一轮。测试必须覆盖 commit 窗口二次写。
- **避坑防重犯建议**：任何增量处理水位都只能推进到“已被本轮处理集覆盖”的输入边界，不能在输出完成后重扫并默认全吃。

### 71. 路径白名单不等于文件对象安全：symlink/junction 可把目录外秘密带进历史版本

- **问题现象与错误原因**：仅检查相对路径属于 `state/`/`memo/`，随后 `os.Open/ReadFile` 会跟随链接，导致白名单目录里的 symlink/reparse point 把数据根外文件复制进 `.snapshots`。
- **排查过程**：真实创建白名单内链接指向目录外文件，WalkDir 给出合法字面路径，但文件打开落到了外部对象。
- **正确做法与标准修复方案**：统一安全枚举器使用 Lstat，仅接受 regular file，拒绝 symlink/junction/reparse，并做数据根 containment；ScanMtime/fingerprint/backup/git add 前全复用。
- **避坑防重犯建议**：安全边界必须同时验证“路径名”和“最终文件对象”；字符串 startsWith/relative whitelist 只能做第一层。

### 72. 运行中 store 的历史恢复不能直接覆盖正式文件：旧内存会再写回

- **问题现象与错误原因**：直接恢复 config/state 到盘后若应用未立刻退出，旧内存 store 的下一次保存会把历史版本覆盖掉，UI 虽提示重启也无法保证结果存活。
- **排查过程**：恢复 config 后继续改任意设置，Store.Update 基于旧副本全量保存，历史内容消失。
- **正确做法与标准修复方案**：非 memo 恢复写进 `.snapshots/pending-restore`，下次启动在任何 Store/模块构造前校验摘要并原子应用；失败 fail loud 且保留 pending 重试。memo 可走已有受控热恢复。
- **避坑防重犯建议**：有内存权威副本的文件恢复，要么热 reload 并阻断旧写，要么延迟到下一进程加载前提交，不能只“写盘+提示重启”。

### 73. 系统资源与配置双写必须有补偿事务，不能把两边各当一次成功

- **问题现象与错误原因**：热键换绑/停用涉及 OS 注册表、Registry 槽位、内存配置、磁盘配置；任一中途失败若先删记账或让调用方单边回滚，会留下幽灵热键、页面显示旧键而系统生效新键。
- **排查过程**：注入新键注册成功、旧键注销失败；以及系统换绑成功、配置保存失败，旧实现均出现三态分叉。
- **正确做法与标准修复方案**：完整操作串行化；Unbind 仅在 OS 注销成功后删槽位；保存失败补偿恢复旧系统态，补偿失败用 errors.Join 明示不一致并保留重试凭据。
- **避坑防重犯建议**：跨 OS/磁盘的“两阶段操作”必须先定义成功/失败不变量和补偿；先改内存再持久化通常是错的。

### 74. CommandContext 只会发取消，不保证调用方按预算返回：cmd.Wait 必须单一所有者且有界

- **问题现象与错误原因**：自检超时后即使 Kill 子进程，若后代继承 stdout/stderr 管道，随后同步 `cmd.Wait()` 仍可能无限等 EOF，所谓 15s 硬预算失效。
- **排查过程**：构造 helper child 延迟退出/继承管道，probe 已超时但 Wait 不归。
- **正确做法与标准修复方案**：唯一 wait goroutine；协调器 select probe/process/context；超时 Kill+关管道，只等固定收尸窗口，超时直接返回，不再同步 Wait；设置 WaitDelay。
- **避坑防重犯建议**：一个 `exec.Cmd` 只能有一个 Wait 所有者；所有超时链都要把“进程终止”和“管道收口”分别设预算。

### 75. USB BusID 是拓扑位置不是物理身份，自动重放必须核指纹

- **问题现象与错误原因**：设备拔出后另一台设备插同端口会复用 BusID；只按 BusID 自动 bind/attach 会把错误硬件交给 WSL。
- **排查过程**：账本 A=2-3/VID-A，现态同 2-3/VID-B，旧计划仍生成动作。
- **正确做法与标准修复方案**：账本存 InstanceID/Serial/Guid/VID-PID；BusID 命中仍核身份，换口只接受唯一稳定匹配，多义 fail closed；旧账本保守兼容。
- **避坑防重犯建议**：端口号、盘符、PID 这类位置/瞬时标识都不能独自承担资源身份。

### 76. JSON payload 不能在 marshal 后硬截，截断必须发生在字段层

- **问题现象与错误原因**：对序列化后的 JSON 字符串直接切 1MiB 再补尾巴，会切在字符串/转义/多字节字符中间，结果不再是合法 JSON。
- **排查过程**：高转义密度 OCR 文本让原字段未超限但 JSON escaping 后超限，旧兜底返回 json.Valid=false。
- **正确做法与标准修复方案**：先裁原始可裁字段并置 truncated，再重新 marshal；通用对象无法安全裁剪则返回合法超限错误信封。测试同时断言 json.Valid、UTF-8、长度预算。
- **避坑防重犯建议**：结构化格式的限长永远在结构层完成；不能把编码后的字节流当普通日志字符串截。

### 77. 发布工件必须 staging→复验→原子替换，直接写最终名会留下残包与旧摘要

- **问题现象与错误原因**：zip 直接创建最终路径，写盘/复验失败会留下截断新 zip，而旧 `.sha256` 仍在；公开包红线正则还曾把真实 `mmmojo_64.dll` 拼少一个 m 而漏检。
- **排查过程**：先成功发布，再在第二次发布注入 zip replace/sidecar 阶段失败；旧实现正式工件被部分覆盖。
- **正确做法与标准修复方案**：明确腾讯资产 basename denylist（大小写/嵌套均拒）；双包先同卷 staging 制备、重开复验、摘要+fsync，批次提交失败恢复旧 zip/sidecar 全字节。
- **避坑防重犯建议**：发布目录只允许出现“上一版完整工件”或“下一版完整工件”，中间态不得使用正式文件名；合规 denylist 测试要用真实资产拼写。
...
### 85. TranslucentTB 2026.2 便携版秒崩 0xC0000005：通用异常退出的"高发起因预告"文案成了误导源——按码分档 + 托管外复现才是归因正道

- **问题现象与错误原因**：真机上托管的 TranslucentTB 2026.2 便携版启动即败，引擎报"异常退出（退出码 3221225477）"。既有话术对所有非零码统一预告两成因（首启欢迎窗关闭＝上游正常退路径 / WinUI·VCLibs 框架包缺失），把机主引向"关过授权窗？缺依赖？"的错误排查方向。实际：3221225477 = 0xC0000005 原生访问违例；而"未同意许可"走的是退码 0，缺框架包另有「缺少依赖」弹窗——两预告成因对该码全部不适用。
- **排查过程**：① 绕开 hanxi 直接 `Start-Process` 同 exe——同一 0xC0000005 秒崩，排除 JobObject/隐藏窗等托管环境嫌疑；② `Get-AppxPackage` 核对依赖：WinUI 2.8（8.2501）与 VCLibs.140.00.UWPDesktop（14.0.33728）在装且够新，排除缺依赖；③ WER 只留着我复现那次的转储（CrashDumps/13656.dmp），事件流无该应用历史 Application Error；④ 上游 GitHub：release 说明自述 2026.2 在"reducing crashes / 避开 XAML 任务栏 ExplorerHooks"，issue 区挂着同类 `0xc0000005 hash-table AV` 报告——判定为上游崩溃线在 22631（XAML 任务栏世代）上的命中，与 hanxi 无关。
- **正确做法与标准修复方案**：hanxi 侧把 `abnormalExitWording` 按退出码分档——0xC0000005 明说"访问违例，非许可/依赖问题"，2026.2 点名上游回归并直给降级指引（装 2026.1/2025.1），其余版本给"第三方任务栏/外壳扩展冲突"排查方向；非 AV 码保留原两成因逐字预告（对退码 1 等仍成立，原测试锚点不动）。用户处置顺序：降级重试 → 旧版仍崩查冲突软件 → 上报上游带转储。
- **避坑防重犯建议**：给"一个信号多种成因"的失败写预告文案时，**每个预告成因必须对该信号成立**——把不适用于访问违例的"体面退出路径"写进同一模板，等于制造假线索；托管外部程序崩溃的归因三步固定为：托管外裸启动复现（排托管环境）→ 依赖/冲突面核对（排本机）→ 上游 release notes/issues 对照（定性版本），三步各自独立便宜，顺序不可倒。
- **后续勘误（同日，机主证词推翻单因定性）**："2026.2 上游已知回归"断言过早——机主反馈**同版本此前一直正常**。追加法医：①重装落盘与 GitHub 当前资产逐字节一致（zip/exe 双哈希对上），传输与解包管线清白；②该 tag 资产仅 8/31 发布当天早/晚替换过一次，此后未动；③TTB 在本机从未留下任何配置（注册表键不存在）——现在连首启都走不完，与"此前正常"并立；④explorer 进程自 9/18 未重启。修正后的立场：同字节、同机器、此前能跑 → 首嫌**本机运行态陈旧**（explorer 内旧一次注入的 TTB hooks DLL 与 18:46 重装落盘的新 DLL 混态，最易致访问违例），次嫌版本回归（上游 issue 同类 AV 仍是旁证但非本案定论）。文案随之改为"重启→降级→按旧版能否跑鉴别归因"的**动作序列**而非归罪断言。教训加一条：**上游 issue 挂着同类崩溃 ≠ 本案就是那个原因**，机主"以前是好的"这类反证证词优先级高于任何远程检索结论。
- **最终定位（同日晚，转储法医）**：修复测试版 2026.1.8（维护者 Azure build 188）在本机同样秒崩 0xC0000005，排除"单版本回归"；minidump 解析 RIP=`msvcp140.dll+0x12F58`（System32 正版运行时，RDX=0 空入参），模块清单零第三方 DLL（排除注入），SAC=0/AppLocker 无规则/ASR 无（排除策略），便携版 Authenticode Valid（排除签名拦截）。决定性环境事实：本机挂着 **OrayIddDriver / Todesk Virtual Display Adapter / GameViewer Virtual Display Adapter** 三块虚拟显卡，`Win32_DesktopMonitor` 里两个"默认监视器"空壳（一个连分辨率都为空）。归因：**TTB 未打包启动路径按监视器逐个初始化，撞上虚拟显示器的空枚举值崩在 STL 字符串处理**；商店版走 `Windows.Graphics.Display` 打包 API 不受影响——"便携全死、商店独活"的分界即打包/未打包显示路径之差。处置：需要便携时设备管理器禁用对应虚拟显卡验证恢复；日常直接用商店版；已把"多版全崩而商店版正常→首查虚拟显示器"写进 AV 档话术。法医链沉淀：托管外裸启动复现 → 资产双哈希（排管线）→ minidump 模块表（排注入/定凶器）→ 设备枚举（定根因），全程零 GUI 调试器，python `minidump` 包 30 行手解 CONTEXT_AMD64（Rip@0xF8/Rsp@0x98）即可。

### 86. VTU `stubs: { teleport: true }` 把弹窗变成死 DOM：插槽内 reactive 依赖无人追踪，handler 全触发但视图永不更新（N31 一键直通选择框）

- **问题现象与错误原因**：WslUsbPanel「一键直通」多发行版选择框（Teleport to body + v-model radio + `:disabled="!pickChoice"` 提交钮）单测中：`radio.setValue()` 后提交钮 `disabled` 永远不放行，直通链卡在 `await askDistroChoice()`。真机构建无此症。根因：`@vue/test-utils` 的 teleport 桩以无状态/函数式替身渲染默认插槽，插槽内容里读取的响应式依赖（pickChoice 等）不归属任何会被失效重渲染的渲染 effect——事件照常派发、ref 照常写入，但那段 DOM 从此不再 patch，等于渲染了一次就冻死的死 DOM。UiPromptDialog.spec 同用此桩却从未暴露，因为它的断言不依赖插槽内二次重渲染。
- **排查过程**：`Object.getOwnPropertySymbols(input)` 摸到 vModelRadio 写入的 `_assign` 符号并包一层再派发原生 change——日志证明 assigner 带着 'Debian' 被调用、`pickChoice.value` 已是新值，而 DOM 的 disabled 纹丝不动；换临时探针 `@change="dbg(); pickChoice = ..."`（方法体内确认 model 已变）后彻底排除事件链与 v-model 本身，锁定重渲染断链；同构 template 组件（真运行时编译、无 Teleport）对照组全绿，嫌疑收敛到桩。
- **正确做法与标准修复方案**：含 Teleport 弹窗的组件 spec 一律不打 teleport 桩——`mount(..., { attachTo: document.body })`，弹窗断言走 `document.body.querySelector('[role="dialog"]')` 直查，交互用原生派发（`radio.checked = true; radio.dispatchEvent(new Event('change', { bubbles: true }))`、`button.click()`），并在 `beforeEach` 清 `document.body.innerHTML = ''`（VTU 不自动卸载上一用例的挂载）。WslUsbPanel.spec 的选择框三用例即按此形态落地（15/15 绿）。
- **避坑防重犯建议**：凡"事件 handler 确认触发、ref 确认更新、视图却不动"的测试假红，第一顺位怀疑渲染上下文被桩/替身切断（teleport stub 尤甚），别再往 v-model/事件模型上找原因；新组件带 Teleport 时，写 spec 前先看 WslUsbPanel.spec 的 body 直查范式，勿复制 UiPromptDialog.spec 的桩范式（后者只在"插槽内容无重渲染需求"时才安全）。

### 87. 表格行内 absolute 下拉被祖先 overflow 容器整片裁剪：菜单要逃 DOM 链（Teleport + fixed 坐标），测这类组件还要先卸壳再清 body（N40 机主实跑反馈）

- **问题现象与错误原因**：WSL「💻 本机发行版」表行内「⋯ 更多」点开后面板被切掉半截（下半屏的行最重）。根因两层：① 面板是挂在行内 `.row-menu`（relative）下的 absolute 子层，而祖先 `.table-container` 有 `overflow-x: auto`——CSS 规范下 overflow-x 非 visible 会把另一轴的 visible 也折算成 auto，于是纵向照样裁剪；② 就算容器不裁，应用主内容区 `.content-area`（overflow-y:auto 的全页滚动壳）仍是裁剪祖先，面板绝对定位再怎么调 z-index 也出不了这个包含块链。
- **排查过程**：从「遮挡」反馈直读 WslDistroTable 菜单实现，顺全局样式 `:where(.table-container)` 与 App.vue `.content-area` 的 overflow 声明钉死两层裁剪源；仓内无同类「视口坐标菜单」先例（QuickMenuPopup 的 getBoundingClientRect 是独立弹出窗口，不算），WslUsbPanel 的 Teleport-to-body 弹窗语系可复用。
- **正确做法与标准修复方案**：小菜单不引 popper 库（零依赖红线），按「Teleport 到 body + `position: fixed`」修：开面板时以触发钮 `getBoundingClientRect()` 算视口坐标；先离屏隐藏（left:-9999px + visibility:hidden）挂载，`nextTick`（微任务，先于浏览器绘制，无闪位）量 offsetWidth/Height 后写终坐标——下方空间不够且上方放得下即向上翻转（`.up` + bottom 锚定），左右向视口内钳制；打开期间 window `scroll`（capture:true，收全部祖先容器）/`resize` 一律收合（fixed 坐标瞬移失真），列表复采（props.instances 换引用，行位移动）同样收合。外点判定要单独认账 teleported 面板本体：pointerdown 命中 `.row-menu` 或 `.row-menu-panel` 之外才收，否则面板内点击在 pointerdown 阶段就被收掉、click 永远到不了菜单项。测试侧：spec 里 mount 过的 wrapper 必须 `afterEach` 先 `unmount()` 再清 `document.body.innerHTML`——只清 DOM 不卸载，残活组件带着开着的菜单等下一次 patch，会打在已被移除的 teleport 锚点上抛错并炸断 Vue 调度队列（同 flush 内其它组件的更新被殃及，出现跨用例假红 + unhandled rejection）。
- **避坑防重犯建议**：凡「行内/卡内 absolute 弹层」先问一句祖先链上有没有 overflow 非 visible 的容器（表格滚动壳、内容区、卡片圆角裁剪都是惯犯）——有就当场定 Teleport+fixed，别指望 z-index，层高低≠被裁剪，二者病因药方完全不同；fixed 弹层必须自带锚点失效收合（scroll/resize/数据复采三处），否则真机上一滚页面菜单就"孤悬视口"。Teleport 组件写 spec 时清场动作固定为「先卸壳、再清 body」两步，只清 body 是半成品（#86 同族坑的镜像形态：#86 说桩会冻死插槽，本条说残挂载会炸队列）。

### 88. winget 代装成功后 hanxi 却仍显示"未安装"：运行中进程看不到机器级安装改写的 PATH（环境块定格），取消语义还有 HRESULT 有/无符号两副面孔（N31 方案 B）

- **问题现象与错误原因**：给 usbipd-win 补真·一键 winget 代装时，"装完即见"链路天然断裂——winget/MSI 机器级安装只把安装目录写进**注册表里的系统 PATH**并广播 `WM_SETTINGCHANGE`，而 Windows 进程的环境块在**创建时定格**，已在运行的 hanxi（及其派生的 `exec.Command("usbipd", …)`）永远看不到新 PATH：`usbipd --version` 依旧 `exec.ErrNotFound`，`GetUsbOverview` 依旧回 installed=false，用户装完却见页面"无动于衷"。第二处暗礁：UAC 被取消时 winget 的退出码在不同呈现下有 `1223`、`0x800704C7`（无符号 2147943623 / 有符号 -2147023673）数副面孔，只按一种判码会把"取消"漏归类成"失败"甚至误判。
- **排查过程**：对照上游 usbipd-win 安装器（MSI 固定落位 `%ProgramFiles%\usbipd-win\`）与 Windows 环境块语义确认"注册表 PATH ≠ 进程 PATH"；仓内既有引导文案（"新开一个窗口即可生效"）即同病灶的旧 workaround 证词。取消码形态按 winerror.h（ERROR_CANCELLED=1223）与 HRESULT 合成式（0x80070000|1223）双向推演，并参考本仓 `runElevatedProcess` 对 1223/文案的双通道判据先例。
- **正确做法与标准修复方案**：`usbipd.Runner` 内置双通道解析——先按名字走 PATH，命中 `exec.ErrNotFound` 且尚未粘住绝对路径时回查 MSI 固定落位（`ProgramFiles`/`ProgramW6432` 双键防 32 位视角错位），存在即改挂绝对路径**粘住**；**未命中不粘滞**（用户运行期间外部装机后下一轮探测自动自救）。代装出口（`WingetInstaller.Install`）与安装态复核（`InstallUsbipdViaWinget` 复验 `usbRun.Version`）都走同一自救通道，"退出码 0"绝不直接换算成功。取消归因把数种码形与中英文案一并收进 `isCancelledExit`，落成独立的 `cancelled` 态（回执"已取消 UAC 授权，未安装"），与 failed 分轨。winget 存在性探测用 `winget list --id … -e` + "输出含钉死的包 ID"判命中（表格本地化换语言不换 ID，"找不到"话术不含 ID），比抠 list 的专属错误码稳。
- **避坑防重犯建议**：凡"hanxi 发起/旁观、由**其它安装器**落位的机器级软件"（winget、MSI、Store 应用），探测通道一律不允许只按 PATH 名字查找——要么内置固定落位兜底，要么读注册表安装信息；给用户的"装完点刷新"类承诺写代码前先自问：**运行中的我这个子进程到底能看见什么环境变量？** 同理，判定 Windows 子进程"被用户取消"永远别只比对一种数值形态（1223 / HRESULT 有无符号 / 本地化文案三类至少覆盖两类以上），且取消必须独立于"失败"成态——把取消混进失败会让 UI 建议用户"重试排障"，混进成功就是 #37 同族谎报。

### 89. 时序竞态修复的测试"假锁"：收到 done 事件后再读状态，回归换序照样绿——必须在广播位同步快照
- **问题现象与错误原因**：修"done 事件先于 SetActive 落账"竞态族（21 托管件）时，竞态一组首版测试写成"监听 done 通道事件→测试 goroutine 补读 store.GetActive() 断言"。负对照（把生产代码 settle/probe 故意换序）实测**这种写法照样绿**——因为读发生在赛跑之后，无论正确时序被怎么破坏，补读到的都是最终落账值，测试根本没锁到"广播瞬间"这个竞态判据点。
- **排查过程**：竞态一族六模块同型修复需要可证伪的回归锁；首版 harness 在 keyviz 上做负对照即暴露失效（换序不红），确认断言点错位而非产品码问题。
- **正确做法与标准修复方案**：在 service 结构体注入 `downloadProbe` 接缝，**位置钉在 Wails Emit 的精确广播点之前**（settle→probe→Emit），探针内**同步**快照 store.GetActive() 并随票据过 channel（channel 收发建立 happens-before，无忙等确定性强）；断言"广播位快照 == 归一化目标版本"+"二次下载快照不被改写"两层。六模块终版全部跑负对照：换序后 6/6 确定性 FAIL、复原 6/6 PASS，才算真锁。生产恒不注入这两个 seam 字段（类型即约束，注释焊死）。
...
- **问题现象与错误原因**：机主报"软件版本雷达找不到微信"。断点不在扫描源：实机 `HKLM\SOFTWARE\WOW6432Node\...\Uninstall\Weixin`（DisplayName=微信/4.1.15.9）页内 probeLocal 三读数一致命中、包内真机冒烟测试原样绿。真断点在全局"可用更新"链：装配根 collectUpdateCheckers 靠 `extapi.UpdateChecker` 类型断言收集感知模块；当日实扫复核托管面达标约 23/28+softver（2026-09-26 晚批勘误：初稿所注"16/19"系记忆错数、所引"§5 #8"实为进程树议题与雷达无关，据实更正），softver 从未实现——雷达轮(state/updates.json)里微信永远缺席是**覆盖面断线**，不是扫描 bug。
...
### 91. 自造 PE 解析器"夹具全绿、真件全红"：资源目录数组偏移把 #2 当 #1（差 8 字节拿到导入表）
- **问题现象与错误原因**：peicon 首版真机集成（RAMMap64.exe）报"目录条目数异常: 27859"，表驱动夹具却全绿。根因：IMAGE_OPTIONAL_HEADER 数据目录数组里 RESOURCE 是**第 2 项**，每项 8 字节，正确偏移 = dataDirOff+16；解析器与测试夹具**同写 dataDirOff+8（那是 IMPORT 目录）自洽错配**——夹具喂给解析器的坏值和解析器的坏读严丝合缝，测试天然测不出。真件里 +8 读到导入表 RVA/Size，被当成资源段根去解析目录头，计数全垃圾。
- **排查过程**：夹具路径修无可修，转而 dump 真 PE 的节表/目录三件套（lfanew、magic、numDir、各节 va/rawPtr）逐字段对官方结构表；0x5629c 落不进 .rsrc 节范围立即露馅。
- **正确做法与标准修复方案**：目录项常量按 `optStart + dataDirOff + kind*8` 参数化并注释索引语义（0=Export 1=Import 2=Resource）；**自研二进制格式解析器必须先过真件冒烟再写夹具表驱动**——夹具只能锁"解析器与自家布局共识"，锁不了"与 Windows 链接器共识"。
- **避坑防重犯建议**：凡"测试夹具 + 手写字节布局"配套开发的协议/格式解析，交付清单里必须有一枚 skip-if-absent 的真文件集成用例当仲裁庭；夹具与解析器共享同一常量文件时，同错互证是结构性盲区，真件是唯一独立证据源。

### 92. PE 图标"按组目录取最大画幅"给 RAMMap 砍半：32² 帧根本不在 RT_GROUP_ICON 成员里
- **问题现象与错误原因**：peicon 初版照"shell 主图标同源"直觉实现"优先解析 RT_GROUP_ICON 首组成员、取成员内最大"，真机 RAMMap64.exe 出货 16×16——而 PS 提取脚本通道（EnumResourceNames 全量 RT_ICON）实证可得更小砍半以外的 32²。dump 资源树坐实：组目录只登记 16² 4bpp 帧，32² 帧是**组外孤儿 RT_ICON**（上游手工拼资源的常见劣化形态）。组优先早退直接丢掉了候选全集。
