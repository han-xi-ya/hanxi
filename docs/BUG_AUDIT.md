...
> 当前状态：部分已修复——本清单为 2026-09-04 审查基线，各条目以行内状态注记为准；  
> 09-08 修复轮总览见 `PROJECT_REVIEW_2026-09-08.md`「修复进展」，未加注记的条目仍属待修复。  
...
> ✅ **核心已修复（2026-09-14）**：`authGate` 鉴权中间件覆盖全部 `/api/*` 数据面（登录/公共配置白名单放行），
> 口令与会话值均恒定时间比较；访客页新增口令门禁，会话 HttpOnly + SameSite=Lax Cookie 30 天，
> 另留 Bearer 口令直连通道；通配 CORS 已移除。回归测试 `internal/modules/fileshare/auth_test.go`。
> **残留**：服务仍绑定 `0.0.0.0`——局域网共享产品形态需要多网卡可达，收敛为网卡白名单待产品决策，不在本次修复范围。
...
- [x] BUG-001 FileShare 鉴权（2026-09-14 authGate + 会话 Cookie + 恒时比较 + 移除通配 CORS；`0.0.0.0` 绑定保留待决策）
- [x] BUG-002 FileShare 真实路径沙箱（09-08 修复轮 B02：全链 `os.Root` 真实路径锚定）
- [ ] BUG-003 第三方镜像独立信任链
