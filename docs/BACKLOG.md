...
### F1 · 统一历史记录公共包 —— 🔵（`feat/f1-history` 分支已按 PLAN 七提交交付，待协调合入后转 🟢）
...
### F6 · 数据目录策略改造：同级强制、退出用户目录、支持绑定 —— ⚪ 建议首批执行
...
### F7 · hanxi-ocr 托管化改造（双引擎 zip 分发体系）—— ⚪ 两端都要动
...
### F8 · 桌面留言板（msgboard，参考 MooTool messageBoard）—— ⚪ 2026-09-17 用户指定排期
...
| S2 | everything 的 MCP 文案如实引导（ES 依赖运行中实例，勿承诺代拉起） | ⚪ 随 F4 落地 |
