...
- **官方版本管理与发行版**：microsoft/WSL Releases 列表 × 本机版本关系判定（过期缓存 stale 标注）；MSI 由**应用内下载器**直落系统"下载"文件夹（进度条 + 打开位置，不经过浏览器），并按本机架构自动标记"本机"、排序首选，异架构仅作备选；在线发行版清单白名单一键安装——但装前先过虚拟化装载门控：虚拟机平台未启用、或"已启用待重启生效"（CBS `RebootPending`）时 WSL2 起不了虚拟机，安装注定失败，此时前端直接亮黄条预告并禁用按钮、后端提权前拦下指路，不再让用户对着闪退的黑窗口反复点"成功"。
